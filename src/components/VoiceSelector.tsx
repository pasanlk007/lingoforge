...
function getVoiceKey(langCode) { return `tts_voice_${langCode}`; }
function getSavedVoice(langCode) { return localStorage.getItem(getVoiceKey(langCode)) || ''; }
function saveVoice(langCode, voiceName) {
  if (voiceName) localStorage.setItem(getVoiceKey(langCode), voiceName);
  else localStorage.removeItem(getVoiceKey(langCode));
}

const RATE_LABELS = { 0.5: 'Very Slow', 0.7: 'Slow', 1.0: 'Normal', 1.25: 'Fast', 1.5: 'Very Fast' };
...
export function VoiceSelector() {
  ...
  const handleVoiceChange = (voiceName) => {
    ...
    toast({ title: 'Voice Saved', description: `Voice for ${langName} set to ${voiceName || 'Default'}.` });
  };
            ...
            <SelectContent className="max-h-60">
              {LINGOFORGE_LANGUAGES.map(lang => (
                <SelectItem key={lang.code} value={lang.code}>
                  {lang.name}{getSavedVoice(lang.code) ? ' ✓' : ''}
                </SelectItem>
              ))}
            </SelectContent>
          </Select>
