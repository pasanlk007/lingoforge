'use client';

// This file contains all translation data and language constants for the app.

export const translations = {
  English: {
    scenario: {
      pageTitle: "Scenario Mode",
      pageDescription: 'Describe your real-life situation — AI builds a daily conversation plan that fits it. (e.g. "I\'m starting a waiter job at a restaurant in Italy", "Preparing for a work permit embassy interview in Romania")',
      myPlans: "My Plans",
      situationLabel: "Your situation",
      situationPlaceholder: "Describe your situation here...",
      targetLanguageLabel: "Target Language",
      nativeLanguageLabel: "Native Language",
      generateButton: "Generate My Plan",
      generatingButton: "Building your plan...",
      errorTooShort: "Please describe your situation in a bit more detail (at least a few words).",
      errorLoginFirst: "Please log in first.",
      errorGenerateFailed: "Couldn't generate the plan. Please try again.",
      lockedTitle: "Scenario Mode",
      lockedDescription: "A custom AI conversation plan for your exact situation — embassy interview, job abroad, food delivery, anything. Subscribe monthly to generate as many plans as you like.",
      lockedFeature1: "Describe your own situation and get a custom plan",
      lockedFeature2: "Daily AI voice conversation practice",
      lockedFeature3: "Generate as many new scenario plans as you like (monthly subscription)",
      subscribeButton: "Subscribe — $13/month",
      myPlansTitle: "My Scenario Plans",
      myPlansSubtitle: "All the plans you've created",
      myPlansEmpty: "You haven't created a plan yet.",
      newPlanButton: "New Plan",
      continueButton: "Continue",
      reviewButton: "Review",
      generatingStatus: "Building plan...",
      errorStatus: "Couldn't generate",
      buildingPlanTitle: "AI is building your plan...",
      buildingPlanSubtitle: "This takes a few seconds.",
      buildFailedTitle: "Couldn't generate the plan",
      tryAgainButton: "Try again",
      slowGenerationTitle: "This is taking longer than usual",
      slowGenerationSubtitle: "There may be a server delay. Please try again.",
      dayPlanLabel: "Day",
      todayBadge: "Today",
      startConversation: "Start Conversation",
      completedBadge: "Completed",
      planCompleteTitle: "Your {days}-day plan is complete! 🎉",
      planCompleteDescription: "You now have basic confidence in {language}. Next step: join our structured Pro Path for deeper, structured lessons.",
      exploreProPath: "Explore Pro Path",
      newScenarioButton: "Create a New Scenario",
      buildingDayTitle: "Building Day {day} content...",
      buildingDaySubtitle: "This takes a few seconds.",
      dayLoadFailedTitle: "Couldn't load Day {day} content",
      subscriptionRequiredTitle: "A subscription is required to unlock this day",
      subscriptionRequiredDescription: "Your plan was created, but you need an active Scenario Mode subscription to generate daily content.",
      phraseCounter: "Phrase {current} / {total}",
      recordPrompt: "Tap to record",
      recordingPrompt: "Speak now... tap stop when done",
      youSaid: "You said:",
      matchConfidence: "~Match confidence:",
      matchDisclaimer: "(This isn't pronunciation accuracy — it's an estimate of how clearly Whisper understood your speech as matching the target text)",
      retryNeeded: "Try again — you need a score above {min}% to continue.",
      micPermissionError: "Couldn't access the microphone. Please check your browser permissions.",
      transcribeError: "Couldn't get the transcription. Please try again.",
      nextPhrase: "Next Phrase",
      dayComplete: "Complete Day {day}",
      subscriptionStatusActive: "Your Scenario Mode subscription is active.",
      subscriptionStatusInactive: "You don't have an active Scenario Mode subscription yet.",
      subscriptionExpiry: "Next renewal/expiry:",
      cancelHowTitle: "How do I cancel my subscription?",
      cancelStep1: '1. Check the receipt email from LemonSqueezy sent when you subscribed — it has a "Manage your subscription" link.',
      cancelStep2: "2. Or go to app.lemonsqueezy.com/my-orders and log in with the email you used at checkout.",
      cancelStep3: "3. From there you can cancel anytime — you won't be charged for the next billing cycle.",
    },
    languagesAvailable: "Languages Available",
    heroTitle: "Developing Language Skills for Migrant Workers",
    heroSub: "Structured language survival paths for migrant workers.",
    startBtn: "👉 Start Now",
    viewPaths: "View Available Languages",
    journeyTitle: "Your Journey Starts Here",
    journeyLeaving: "Leaving home",
    journeyArriving: "Arriving",
    journeyWorking: "Working",
    journeySpeaking: "Speaking",
    journeyBelonging: "Belonging",
    redesign_title: "Just 10 minutes a day can change everything.",
    redesign_subtitle: "10 minutes a day — speak confidently in a foreign country in 84 days.",
    redesign_cta_text: "The language that makes life easier in a foreign country.",
    redesign_cta_button: "Start here",
    poster: {
        title: "Essential Sentences - Systematic Learning Path",
        subtitle: "For daily life • For work",
        tagline: "unfold your opportunities"
    },
    scenario1Title: "Emergency",
    scenario1Desc: "Your child is sick. You need to explain symptoms to a doctor. Do you know the words?",
    scenario1Btn: "Learn Medical Vocab",
    scenario2Title: "Bank & Money",
    scenario2Desc: "You need to open a bank account. The form is in a foreign language. Can you fill it in?",
    scenario2Btn: "Learn Numbers & Docs",
    scenario3Title: "Daily Life",
    scenario3Desc: "You need to take the right bus to work. The driver speaks no English. Can you ask for help?",
    scenario3Btn: "Learn Survival Phrases",
    path1Badge: "START HERE - Recommended",
    path1Title: "Survival Path",
    path1Desc: "Essential phrases to survive from day one in a new country.",
    path2Title: "Alphabet Path",
    path2Desc: "Read signs, menus and messages in the local language.",
    path3Title: "Numbers Path",
    path3Desc: "Handle money, time and numbers confidently every day.",
    pathDetails: "Structured lessons for practical use.",
    startFree: "Start Free",
    getStarted: "Get Started",
    features: {
      ai: { title: "AI-Powered", desc: "Real lessons generated for you" },
      audio: { title: "Audio Built-in", desc: "Hear native pronunciation for every word" },
      dialogues: { title: "Real Dialogues", desc: "Actual conversations you will use daily" },
      exercises: { title: "Daily Exercises", desc: "Fill blanks, MCQ, word matching games" },
      streak: { title: "Streak System", desc: "Stay motivated with daily streaks" },
      progress: { title: "Progress Tracking", desc: "See how far you have come every week" },
      certs: { title: "Certificates", desc: "Earn proof of your progress" },
      langs: { title: "21 Languages", desc: "European, Asian, Middle Eastern" },
    },
    pricingTitle: "Unlock Your Full Potential",
    pricingSub: "Your free trial includes Week 1 of the Survival Path. Upgrade to unlock all lessons.",
    pricingPage: {
        purchasingFor: "You are purchasing for:",
        changeLanguageNote: "Change your target language on the dashboard.",
        autoRenewNote: "This plan auto-renews. Cancel anytime.",
        freeTrialNote: "Access Week 1 of the Survival Path for free. No credit card required.",
    },
    freePlan: {
      title: "Free Plan",
      description: "Access Week 1 of the Survival Path for free. No credit card required.",
      feat1: "Week 1 of Survival Path",
      feat2: "5 new words per day",
      btn: "Start for Free"
    },
    weeklyPlan: {
      title: "Weekly Plan",
      badge: "Popular",
      price_usd: "$3.99/week",
      price_lkr: "LKR 1,200/week",
      feat1: "Access to one chosen language",
      feat2: "All 3 learning paths for that language",
      feat3: "All interactive exercises & audio",
      feat4: "Cancel anytime",
      note: "Payments stop automatically after 12 weeks.",
    },
    completePlan: {
      title: "Survival Pack",
      badge: "Value",
      price_usd: "$26 one-time",
      price_lkr: "LKR 11,700",
      feat1: "Survival + Alphabet + Numbers paths",
      feat2: "Lifetime access to one language course",
      feat3: "Full 12-week curriculum",
      feat4: "Email support",
    },
    lifetimePlan: {
      title: "Lifetime Pro",
      badge: "⭐ Best Value",
      price_usd: "$49 one-time",
      price_lkr: "LKR 29,700",
      feat1: "Everything in Survival Pack + Pro lessons",
      feat2: "Lifetime access to ALL current & future languages",
      feat3: "Priority Support",
      feat4: "Become a polyglot!",
      feat5: "Free future updates",
    },
    footerTagline: "Language survival for the modern migrant.",
    footerLinks: {
      company: "Company",
      paths: "Paths",
      pricing: "Pricing",
      privacy: "Privacy Policy",
      terms: "Terms of Service",
      language: "Language",
    },
    footerCredit: "© 2026 LingoForge. Built for migrants, by people who understand the journey.",
    dashboard: {
      title: "Dashboard",
      welcome: "Welcome back",
      ready: "Ready to continue your language journey? Let's do this.",
      currentStreak: "Current Streak",
      keepFlame: "Keep the flame alive!",
      xpPoints: "XP Points",
      toNextLevel: "to Level",
      level: "Level",
      advancing: "You're advancing!",
      currentPath: "Current Path",
      language: "Language",
      continueJourney: "Continue Your Journey",
      continueDesc: "You're on the {path} path in {language}. Pick up where you left off.",
      nextLesson: "Next Lesson: Week {week}, Day {day}",
      keepProgress: "Keep making progress!",
      goToNextLesson: "Go to Next Lesson",
      weeklyProgress: "This Week's Progress",
      days: {
        mon: 'Mon',
        tue: 'Tue',
        wed: 'Wed',
        thu: 'Thu',
        fri: 'Fri',
        sat: 'Sat',
        sun: 'Sun',
      },
      reminders: {
        title: "Study Reminders",
        description: "Set a daily notification to stay on track with your learning.",
        dailyReminder: "Daily Reminder",
        permissionNeeded: "Notification permission is required to enable reminders.",
        grantPermission: "Grant Permission",
        save: "Save Reminder",
        saved: "Reminder settings saved!",
      },
      referrals: {
        title: "Refer a Friend",
        description: "Share your code with a friend. When they sign up, you both get a bonus!",
        yourCode: "Your Referral Code",
        copyCode: "Copy Code",
        copied: "Copied!",
        bonus: "You both get 1 week of free premium access!"
      },
      iSpeak: "I speak",
      iAmLearning: "I am learning",
      survivalBundle: {
        title: "Survival Bundle",
        description: "Your first step to practical fluency."
      },
      alphabetPath: "Alphabet Path",
      numbersPath: "Numbers Path",
      proBundle: {
        badge: "AI ADVANCED PATH",
        title: "LingoForge Pro",
        description: "A specialized 30-day integration curriculum for migrant workers.",
        nextLesson: "Next Pro Lesson",
        exploreMap: "Explore Lesson Map",
        items: {
          rights: { title: "Rights & Documents", desc: "Legal and civic guidance" },
          integration: { title: "Integration", desc: "Cultural and social norms" },
          vocab: { title: "Legal Vocab", desc: "Essential legal terms" },
          tutors: { title: "AI Tutors", desc: "Practice with AI" }
        }
      }
    },
    notifications: {
      title: "Time for your LingoForge lesson!",
      body: "Your daily {language} lesson is waiting. Keep your streak going!",
    },
    wordCard: {
      phonetic: 'Phonetic',
      meaning: 'Meaning',
      example: 'Example',
    },
    ui: {
      // survival page
      survivalPath: "Survival Path",
      survivalPathDesc: "A 12-week journey to master essential phrases for daily life.",
      week: "Week",
      locked: "Locked",
      days: "Days",
      completed: "Completed",
      day: "Day",
      upgradePlan: "Upgrade Plan",
      lessonsLocked: "This week's lessons are locked.",
      upgradeToUnlock: "Upgrade to Unlock",
      // lesson page errors
      errorTitle: "Could Not Load Lesson",
      errorWeekNotFound: "Lesson content for Week {week} of the {path} path does not exist. Please create the file at 'public{lessonPath}'.",
      errorDayNotFound: "Could not find data for Day {day} in the file '{lessonPath}'. Make sure the 'days' array in the JSON file contains an entry for this day.",
      errorInvalidJson: "The lesson file at '{lessonPath}' contains invalid JSON. Please check the file for syntax errors. Error: {error}",
      errorGeneric: "Failed to load lesson file. Status: {status}",
      errorInvalidDay: 'Invalid day parameter: "{day}". Must be a number.',
      backToDashboard: "Back to Dashboard",
      contentNotAvailableTitle: "Content Not Available",
      contentNotAvailableDesc: "This lesson has not been created yet. You can create the lesson file in the `public/lessons` directory. Please see `docs/lesson-generation-guide.md` for instructions.",
      // lesson client page
      weekProgress: "WEEK PROGRESS",
      dayProgress: "DAY PROGRESS",
      vocabulary: "Vocabulary",
      noVocabulary: "No vocabulary for this lesson.",
      dialogues: "Dialogues",
      tipsAndCulture: "Tips & Culture",
      pronunciationTip: "Pronunciation Tip",
      culturalNote: "Cultural Note",
      dayComplete: "Day Complete!",
      earnedXP: "You've earned {xp} XP and a {streak_bonus} streak bonus.",
      completeDay: "Complete Day (+{xp} XP)",
      complete50Percent: "Complete at least 50% of exercises to finish.",
      // exercises
      practiceExercises: "Practice Exercises",
      fillInTheBlanks: "Fill in the Blanks",
      correct: "Correct!",
      matchingPairs: "Matching Pairs",
      greatJob: "Great job!",
      allPairsMatched: "All pairs matched correctly!",
      arrangeTheWords: "Arrange the Words",
      translate: 'Translate: "{hint}"',
      check: "Check",
      reset: "Reset",
      correctSentence: 'Correct! The sentence is: "{sentence}"',
      tryAgain: "Not quite. Try again!",
      // navigation
      pathsAndFeatures: "Paths & Features",
      logIn: "Log In",
      signUp: "Sign Up",
      profile: "Profile",
      logOut: "Log Out",
    }
  },
  Sinhala: {
    scenario: {
      pageTitle: "Scenario Mode",
      pageDescription: 'ඔබේ real-life situation එක කියන්න — ඊට ගැලපෙන daily conversation plan එකක් AI එක හදනවා. (e.g. "මම ඉතාලියේ රෙස්ටුරන්ට් එකක වේටර් රැකියාවට යනවා", "රුමේනියාවේ work permit embassy interview එකට සූදානම් වෙනවා")',
      myPlans: "My Plans",
      situationLabel: "ඔබේ situation එක",
      situationPlaceholder: "මෙතන ඔබේ situation එක විස්තර කරන්න...",
      targetLanguageLabel: "Target Language",
      nativeLanguageLabel: "Native Language",
      generateButton: "My Plan එක Generate කරන්න",
      generatingButton: "Plan එක හදනවා...",
      errorTooShort: "කරුණාකර ඔබේ situation එක ටිකක් විස්තර කරන්න (අවම වශයෙන් වචන කීපයක්).",
      errorLoginFirst: "කරුණාකර පළමුව login වෙන්න.",
      errorGenerateFailed: "Plan එක generate කරන්න බැරි උනා. ආයෙත් try කරන්න.",
      lockedTitle: "Scenario Mode",
      lockedDescription: "ඔබේම real-life situation එකට ගැලපෙන AI conversation plan එකක් — embassy interview, job abroad, food delivery, ඕන situation එකක්ම. මාසික subscription එකකින් ඔබට ඕන තරම් plan generate කරන්න පුළුවන්.",
      lockedFeature1: "ඔබේම situation එක describe කරලා custom plan එකක් generate කරන්න",
      lockedFeature2: "Daily AI voice conversation practice",
      lockedFeature3: "ඕන තරම් අලුත් scenario plans හදන්න (monthly subscription)",
      subscribeButton: "Subscribe කරන්න — $13/month",
      myPlansTitle: "My Scenario Plans",
      myPlansSubtitle: "ඔබ හදපු සියලුම plans",
      myPlansEmpty: "ඔබ තාම plan එකක් හදලා නෑ.",
      newPlanButton: "අලුත් එකක්",
      continueButton: "Continue",
      reviewButton: "Review",
      generatingStatus: "Plan එක හදනවා...",
      errorStatus: "Generate කරන්න බැරි උනා",
      buildingPlanTitle: "ඔබේ plan එක AI එක හදනවා...",
      buildingPlanSubtitle: "මේක seconds කීපයක් ගතවෙයි.",
      buildFailedTitle: "Plan එක generate කරන්න බැරි උනා",
      tryAgainButton: "ආයෙත් try කරන්න",
      slowGenerationTitle: "මේක සාමාන්‍යයෙන් වඩා ගතවෙනවා",
      slowGenerationSubtitle: "Server එකේ delay එකක් වෙන්න පුළුවන්. ආයෙත් try කරන්න.",
      dayPlanLabel: "Day",
      todayBadge: "අද",
      startConversation: "Start Conversation",
      completedBadge: "සම්පූර්ණයි",
      planCompleteTitle: "ඔබේ {days} දින plan එක සම්පූර්ණයි! 🎉",
      planCompleteDescription: "දැන් ඔබට {language} භාෂාවේ basic confidence එකක් තියෙනවා. ඊළඟ step එක: structured, deep-dive lessons සහිත අපේ Pro Path එකට join වෙන්න.",
      exploreProPath: "Pro Path එක explore කරන්න",
      newScenarioButton: "අලුත් Scenario එකක් හදන්න",
      buildingDayTitle: "Day {day} content එක හදනවා...",
      buildingDaySubtitle: "මේක seconds කීපයක් ගතවෙයි.",
      dayLoadFailedTitle: "Day {day} content එක load කරන්න බැරි උනා",
      subscriptionRequiredTitle: "මේ day එක unlock කරන්න subscription එකක් ඕන",
      subscriptionRequiredDescription: "ඔබේ plan එක create උනා, නමුත් daily content generate කරන්න Scenario Mode subscription එක active කරගන්න ඕන.",
      phraseCounter: "Phrase {current} / {total}",
      recordPrompt: "Record කරන්න tap කරන්න",
      recordingPrompt: "කියන්න... finish උනාම stop කරන්න",
      youSaid: "ඔබ කිව්වේ:",
      matchConfidence: "~Match confidence:",
      matchDisclaimer: "(මේක pronunciation accuracy නෙවෙයි — Whisper ට ඔබේ speech එක කොච්චර clear ලෙස target text එකට ලඟ ලෙස තේරුණාද කියන estimate එකක්)",
      retryNeeded: "ආයෙත් try කරන්න — {min}%ට වැඩි score එකක් ගන්න ඕන ඉදිරියට යන්න.",
      micPermissionError: "Microphone access ලබාගන්න බැරි උනා. Browser permissions check කරන්න.",
      transcribeError: "Transcription එක ලබාගන්න බැරි උනා. ආයෙත් try කරන්න.",
      nextPhrase: "Next Phrase",
      dayComplete: "Day {day} Complete කරන්න",
      subscriptionStatusActive: "ඔබේ Scenario Mode subscription එක active.",
      subscriptionStatusInactive: "ඔබට තාම active Scenario Mode subscription එකක් නෑ.",
      subscriptionExpiry: "ඊළඟ renewal/expiry:",
      cancelHowTitle: "Subscription එක cancel කරන්නේ කොහොමද?",
      cancelStep1: '1. ඔබ subscribe උනාම LemonSqueezy එකෙන් එවුන receipt email එක check කරන්න — ඒකේ "Manage your subscription" link එකක් තියෙනවා.',
      cancelStep2: "2. නැත්නම් app.lemonsqueezy.com/my-orders එකට ගිහින් ඔබ checkout එකේ දුන්න email එකෙන් login වෙන්න.",
      cancelStep3: "3. එතනින් subscription එක ඕන වෙලාවක cancel කරන්න පුළුවන් — ඊළඟ billing cycle එකේ සිට charge වෙන්නේ නෑ.",
    },
    languagesAvailable: "භාෂා ඇත",
    heroTitle: "Developing Language Skills for Migrant Workers",
    heroSub: "විදෙස්ගත ශ්‍රමිකයන් සඳහා ව්‍යුහගත භාෂා ඉගැන්වීම් සැකසුම",
    startBtn: "👉 දැන් ආරම්භ කරන්න",
    viewPaths: "පවතින භාෂා බලන්න",
    journeyTitle: "ඔබේ ගමන මෙතනින් ආරම්භ වේ",
    journeyLeaving: "නිවස හැරයාම",
    journeyArriving: "පැමිණීම",
    journeyWorking: "වැඩ කිරීම",
    journeySpeaking: "කතා කිරීම",
    journeyBelonging: "අයිති වීම",
    redesign_title: "දිනකට මිනිත්තු 10ක් සියල්ල වෙනස් කළ හැකිය.",
    redesign_subtitle: "දිනකට මිනිත්තු 10ක් — දින 84කින් විදේශ රටක විශ්වාසයෙන් කතා කරන්න.",
    redesign_cta_text: "විදේශ රටක ජීවිතය පහසු කරන භාෂාව.",
    redesign_cta_button: "Start here",
    poster: {
        title: "අත්‍යවශ්‍ය වාක්‍ය - ක්‍රමානුකූල ඉගෙනුම් මාවත",
        subtitle: "දෛනික ජීවිතයට • රැකියාවට",
        tagline: "unfold your opportunities"
    },
    scenario1Title: "හදිසි අවස්ථාවක්",
    scenario1Desc: "ඔබේ දරුවා අසනීපයි. ඔබට වෛද්‍යවරයෙකුට රෝග ලක්ෂණ පැහැදිලි කිරීමට අවශ්‍යයි. ඔබ වචන දන්නවාද?",
    scenario1Btn: "වෛද්‍ය වචන ඉගෙන ගන්න",
    scenario2Title: "බැංකු සහ මුදල්",
    scenario2Desc: "ඔබට බැංකු ගිණුමක් විවෘත කිරීමට අවශ්‍යයි. පෝරමය විදේශ භාෂාවකින් ඇත. ඔබට එය පිරවිය හැකිද?",
    scenario2Btn: "සංඛ්‍යා සහ ලේඛන ඉගෙන ගන්න",
    scenario3Title: "දෛනික ජීවිතය",
    scenario3Desc: "ඔබට වැඩට යාමට නිවැරදි බසය ගත යුතුය. රියදුරු ඉංග්‍රීසි කතා නොකරයි. ඔබට උදව් ඉල්ලා සිටිය හැකිද?",
    scenario3Btn: "පැවැත්මේ වාක්‍ය ඉගෙන ගන්න",
    path1Badge: "විදේශගතවීමට බලාපොරොත්තුවෙන් සිටින/විදේශගතව සිටින ඔබ අනිවාර්යයෙන් ඉගනිය යුතු ව්‍යුහගත ඉගනීමේ සැලැස්ම",
    path1Title: "පැවැත්මේ මාර්ගය",
    path1Desc: "නව රටක පළමු දිනයේ සිටම දිවි ගෙවීමට අත්‍යවශ්‍ය වාක්‍ය.",
    path2Title: "අකුරු මාර්ගය",
    path2Desc: "සලකුණු, මෙනු සහ පණිවිඩ දේශීය භාෂාවෙන් කියවන්න.",
    path3Title: "සංඛ්‍යා මාර්ගය",
    path3Desc: "සෑම දිනකම මුදල්, වේලාව සහ සංඛ්‍යා විශ්වාසයෙන් හසුරුවන්න.",
    pathDetails: "ප්‍රායෝගික භාවිතය සඳහා ව්‍යුහගත පාඩම්.",
    startFree: "නොමිලේ ආරම්භ කරන්න",
    getStarted: "ආරම්භ කරන්න",
    features: {
      ai: { title: "AI බලයෙන්", desc: "ඔබ වෙනුවෙන් ජනනය කරන ලද පාඩම්" },
      audio: { title: "ශ්‍රව්‍ය ඇතුළත්", desc: "සෑම වචනයක් සඳහාම දේශීය උච්චාරණය අසන්න" },
      dialogues: { title: "සැබෑ සංවාද", desc: "ඔබ දිනපතා භාවිතා කරන සැබෑ සංවාද" },
      exercises: { title: "දෛනික අභ්‍යාස", desc: "හිස්තැන් පිරවීම, MCQ, වචන ගැලපීම" },
      streak: { title: "දිනපතා Streak", desc: "දිනපතා streaks සමඟින් පෙළඹී සිටින්න" },
      progress: { title: "ප්‍රගති නිරීක්ෂණය", desc: "සෑම සතියකම ඔබ කොතරම් දුරක් පැමිණ ඇත්දැයි බලන්න" },
      certs: { title: "සහතික", desc: "ඔබේ ප්‍රගතිය පිළිබඳ සාක්ෂි උපයන්න" },
      langs: { title: "භාෂා 21", desc: "යුරෝපීය, ආසියානු, මැද පෙරදිග" },
    },
    pricingTitle: "ඔබේ සම්පූර්ණ විභවය විවෘත කරන්න",
    pricingSub: "ඔබේ නොමිලේ අත්හදා බැලීමට Survival Path හි 1 වන සතිය ඇතුළත් වේ. සියලුම පාඩම් අගුළු ඇරීමට උත්ශ්‍රේණි කරන්න.",
    pricingPage: {
        purchasingFor: "ඔබ මිලදී ගන්නේ:",
        changeLanguageNote: "ඔබේ ඉලක්ක භාෂාව පාලන පුවරුවෙන් වෙනස් කරන්න.",
        autoRenewNote: "මෙම සැලැස්ම ස්වයංක්‍රීයව අලුත් වේ. ඕනෑම වේලාවක අවලංගු කරන්න.",
        freeTrialNote: "Survival Path හි 1 වන සතියට නොමිලේ ප්‍රවේශ වන්න. ක්‍රෙඩිට් කාඩ් අවශ්‍ය නොවේ.",
    },
    freePlan: {
      title: "නොමිලේ සැලැස්ම",
      description: "Survival Path හි 1 වන සතියට නොමිලේ ප්‍රවේශ වන්න. ක්‍රෙඩිට් කාඩ් අවශ්‍ය නොවේ.",
      feat1: "Survival Path හි 1 වන සතිය",
      feat2: "දිනකට නව වචන 5ක්",
      btn: "නොමිලේ ආරම්භ කරන්න"
    },
    weeklyPlan: {
      title: "සතිපතා සැලැස්ම",
      badge: "ජනප්‍රිය",
      price_usd: "$3.99/සතිය",
      price_lkr: "LKR 1,200/සතිය",
      feat1: "තෝරාගත් එක් භාෂාවකට ප්‍රවේශය",
      feat2: "එම භාෂාව සඳහා ඉගෙනුම් මාර්ග 3ම",
      feat3: "සියලුම අන්තර්ක්‍රියාකාරී අභ්‍යාස සහ ශ්‍රව්‍ය",
      feat4: "ඕනෑම වේලාවක අවලංගු කරන්න",
      note: "සති 12 සම්පූර්ණ කළ පසු ගෙවීම් ස්වයංක්‍රීයව නවතී",
    },
    completePlan: {
      title: "සම්පූර්ණ පාඨමාලාව",
      badge: "වටිනාකම",
      price_usd: "$26 one-time",
      price_lkr: "LKR 11,700",
      feat1: "සතිපතා සැලැස්මේ ඇති සියල්ල",
      feat2: "එක් භාෂා පාඨමාලාවකට ජීවිත කාලය පුරාම ප්‍රවේශය",
      feat3: "සම්පූර්ණ සති 12ක විෂය නිර්දේශය",
      feat4: "විද්‍යුත් තැපැල් සහාය",
    },
    lifetimePlan: {
      title: "Lifetime Pro",
      badge: "⭐ Best Value",
      price_usd: "$49 one-time",
      price_lkr: "LKR 29,700",
      feat1: "සම්පූර්ණ පාඨමාලාවේ ඇති සියල්ල",
      feat2: "වර්තමාන සහ අනාගත සියලුම භාෂා සඳහා ජීවිත කාලය පුරාම ප්‍රවේශය",
      feat3: "ප්‍රමුඛතා සහාය",
      feat4: "බහුභාෂිකයෙකු වන්න!",
      feat5: "නොමිලේ අනාගත යාවත්කාලීන කිරීම්",
    },
    footerTagline: "නූතන සංක්‍රමණිකයා සඳහා භාෂා පැවැත්ම.",
    footerLinks: {
      company: "සමාගම",
      paths: "මාර්ග",
      pricing: "මිල ගණන්",
      privacy: "පෞද්ගලිකත්ව ප්‍රතිපත්තිය",
      terms: "සේවා කොන්දේසි",
      language: "භාෂාව",
    },
    footerCredit: "© 2026 LingoForge. සංක්‍රමණිකයන් සඳහා, ගමන තේරුම් ගන්නා අය විසින් ගොඩනගන ලදී.",
    dashboard: {
      title: "පාලන පුවරුව",
      welcome: "නැවතත් සාදරයෙන් පිළිගනිමු",
      ready: "ඔබේ සාර්ථකත්වයට පදනම — භාෂාවයි.",
      currentStreak: "වර්තමාන Streak",
      keepFlame: "දිනකට අවම වචන 5 පාඩම් කිරීමට උත්සහ කරන්න",
      xpPoints: "XP ලකුණු",
      toNextLevel: "මට්ටමට",
      level: "මට්ටම",
      advancing: "ඔබ ඉදිරියට යමින්!",
      currentPath: "වර්තමාන මාර්ගය",
      language: "භාෂාව",
      continueJourney: "ඔබේ ගමන දිගටම කරගෙන යන්න",
      continueDesc: "ඔබ {language} හි {path} මාර්ගයේ සිටී. ඔබ නැවැත්වූ තැනින් නැවත ආරම්භ කරන්න.",
      nextLesson: "ඊළඟ පාඩම: සතිය {week}, දිනය {day}",
      keepProgress: "දිගටම ප්‍රගතියක් ලබන්න!",
      goToNextLesson: "ඊළඟ පාඩමට යන්න",
      weeklyProgress: "මෙම සතියේ ප්‍රගතිය",
      days: {
        mon: 'සඳු',
        tue: 'අඟ',
        wed: 'බදා',
        thu: 'බ්‍රහ',
        fri: 'සිකු',
        sat: 'සෙන',
        sun: 'ඉරි',
      },
      reminders: {
        title: "අධ්‍යයන මතක් කිරීම්",
        description: "ඔබේ ඉගෙනීම සමඟ නිරතුරුව සිටීමට දෛනික දැනුම්දීමක් සකසන්න.",
        dailyReminder: "දෛනික මතක් කිරීම",
        permissionNeeded: "මතක් කිරීම් සක්‍රීය කිරීමට දැනුම්දීම් අවසරය අවශ්‍ය වේ.",
        grantPermission: "අවසරය ලබා දෙන්න",
        save: "මතක් කිරීම සුරකින්න",
        saved: "මතක් කිරීමේ සැකසුම් සුරකින ලදි!",
      },
      referrals: {
        title: "මිතුරෙකුට යොමු කරන්න",
        description: "👉  කේතය මිතුරෙකු සමඟ බෙදා ගන්න",
        yourCode: "ඔබේ යොමු කිරීමේ කේතය",
        copyCode: "කේතය පිටපත් කරන්න",
        copied: "පිටපත් කරන ලදී!",
        bonus: "ඔබ දෙදෙනාටම සති 1ක නොමිලේ වාරික ප්‍රවේශය ලැබේ!"
      },
      iSpeak: "මම කතා කරන්නේ",
      iAmLearning: "මම ඉගෙන ගන්නේ",
      survivalBundle: {
        title: "Survival Bundle",
        description: "ප්‍රායෝගික චතුරතාව සඳහා ඔබේ පළමු පියවර."
      },
      alphabetPath: "අකුරු හදුනාගනිමු",
      numbersPath: "ඉලක්කම් ඉගෙනගමු",
      proBundle: {
        badge: "AI උසස් මාර්ගය",
        title: "LingoForge Pro",
        description: "විදෙස්ගත ශ්‍රමිකයන් සඳහා විශේෂිත දින 30ක ඒකාබද්ධතා විෂයමාලාව.",
        nextLesson: "ඊළඟ Pro පාඩම",
        exploreMap: "පාඩම් සිතියම ගවේෂණය කරන්න",
        items: {
          rights: { title: "අයිතිවාසිකම් සහ ලේඛන", desc: "නීතිමය සහ සිවිල් මගපෙන්වීම" },
          integration: { title: "ඒකාබද්ධ වීම", desc: "සංස්කෘතික හා සමාජ සම්මතයන්" },
          vocab: { title: "නීතිමය වචන මාලාව", desc: "අත්‍යවශ්‍ය නීතිමය නියමයන්" },
          tutors: { title: "AI උපදේශකයින්", desc: "AI සමඟ පුහුණු වන්න" }
        }
      }
    },
    notifications: {
        title: "ඔබගේ LingoForge පාඩම සඳහා වේලාවයි!",
        body: "ඔබගේ දෛනික {language} පාඩම බලා සිටී. ඔබගේ streak එක දිගටම කරගෙන යන්න!",
    },
    wordCard: {
      phonetic: 'උච්චාරණය',
      meaning: 'තේරුම',
      example: 'උදාහරණය',
    },
    ui: {
      // survival page
      survivalPath: "Survival මාවත",
      survivalPathDesc: "දෛනික ජීවිතයට අත්‍යවශ්‍ය වාක්‍ය ඛණ්ඩ ප්‍රගුණ කිරීමට සති 12ක ගමනක්.",
      week: "සතිය",
      locked: "අගුළු දමා ඇත",
      days: "දින",
      completed: "සම්පූර්ණයි",
      day: "දිනය",
      upgradePlan: "සැලැස්ම වැඩි දියුණු කරන්න",
      lessonsLocked: "මෙම සතියේ පාඩම් අගුළු දමා ඇත.",
      upgradeToUnlock: "අගුළු ඇරීමට උත්ශ්‍රේණි කරන්න",
      // lesson page errors
      errorTitle: "පාඩම පූරණය කළ නොහැක",
      errorWeekNotFound: "{path} මාර්ගයේ {week} වන සතිය සඳහා පාඩම් අන්තර්ගතය නොමැත. කරුණාකර 'public{lessonPath}' හි ගොනුව සාදන්න.",
      errorDayNotFound: "'{lessonPath}' ගොනුවේ {day} වන දිනය සඳහා දත්ත සොයාගත නොහැකි විය. JSON ගොනුවේ 'days' অ্যარე එකේ මෙම දිනය සඳහා ඇතුළත් කිරීමක් ඇති බවට වග බලා ගන්න.",
      errorInvalidJson: "'{lessonPath}' හි ඇති පාඩම් ගොනුවේ වලංගු නොවන JSON අඩංගු වේ. කරුණාකර වාක්‍ය ඛණ්ඩ දෝෂ සඳහා ගොනුව පරීක්ෂා කරන්න. දෝෂය: {error}",
      errorGeneric: "පාඩම් ගොනුව පූරණය කිරීමට අසමත් විය. තත්ත්වය: {status}",
      errorInvalidDay: 'වලංගු නොවන දින පරාමිතිය: "{day}". එය අංකයක් විය යුතුය.',
      backToDashboard: "පාලන පුවරුව වෙත ආපසු",
      contentNotAvailableTitle: "අන්තර්ගතය ලබා ගත නොහැක",
      contentNotAvailableDesc: "මෙම පාඩම තවම නිර්මාණය කර නොමැත. ඔබට 'public/lessons' නාමාවලියෙහි පාඩම් ගොනුව සෑදිය හැක. උපදෙස් සඳහා කරුණාකර `docs/lesson-generation-guide.md` බලන්න.",
      // lesson client page
      weekProgress: "සතියේ ප්‍රගතිය",
      dayProgress: "දවසේ ප්‍රගතිය",
      vocabulary: "වචන මාලාව",
      noVocabulary: "මෙම පාඩම සඳහා වචන මාලාවක් නොමැත.",
      dialogues: "දෙබස්",
      tipsAndCulture: "උපදෙස් සහ සංස්කෘතිය",
      pronunciationTip: "උච්චාරණ උපදෙස්",
      culturalNote: "සංස්කෘතික සටහන",
      dayComplete: "දවස සම්පූර්ණයි!",
      earnedXP: "ඔබ XP {xp} ක් සහ දින {streak_bonus} ක streak bonus එකක් ලබා ගත්තා.",
      completeDay: "දවස සම්පූර්ණ කරන්න (+{xp} XP)",
      complete50Percent: "අවසන් කිරීමට අවම වශයෙන් 50% ක් වත් අභ්‍යාස සම්පූර්ණ කරන්න.",
      // exercises
      practiceExercises: "පුහුණු අභ්‍යාස",
      fillInTheBlanks: "හිස්තැන් පුරවන්න",
      correct: "නිවැරදියි!",
      matchingPairs: "ගලපන යුගල",
      greatJob: "නියමයි!",
      allPairsMatched: "සියලුම යුගල නිවැරදිව ගලපා ඇත!",
      arrangeTheWords: "වචන සකසන්න",
      translate: 'පරිවර්තනය කරන්න: "{hint}"',
      check: "පරීක්ෂා කරන්න",
      reset: "යළි පිහිටුවන්න",
      correctSentence: 'නිවැරදියි! වාක්‍යය නම්: "{sentence}"',
      tryAgain: "සම්පූර්ණයෙන්ම හරි නෑ. නැවත උත්සාහ කරන්න!",
      // navigation
      pathsAndFeatures: "මාර්ග සහ විශේෂාංග",
      logIn: "ඇතුල් වන්න",
      signUp: "ලියාපදිංචි වන්න",
      profile: "පැතිකඩ",
      logOut: "ඉවත් වන්න",
    }
  },
  Hindi: {
    scenario: {
      pageTitle: "सिनेरियो मोड",
      pageDescription: 'अपनी वास्तविक जीवन की स्थिति का वर्णन करें — AI एक दैनिक बातचीत योजना बनाता है जो इसमें फिट बैठती है। (उदा. "मैं इटली के एक रेस्तरां में वेटर की नौकरी शुरू कर रहा हूँ", "रोमानिया में वर्क परमिट दूतावास साक्षात्कार की तैयारी कर रहा हूँ")',
      myPlans: "मेरी योजनाएं",
      situationLabel: "आपकी स्थिति",
      situationPlaceholder: "अपनी स्थिति का वर्णन यहाँ करें...",
      targetLanguageLabel: "लक्ष्य भाषा",
      nativeLanguageLabel: "मातृभाषा",
      generateButton: "मेरी योजना बनाएँ",
      generatingButton: "आपकी योजना बन रही है...",
      errorTooShort: "कृपया अपनी स्थिति का थोड़ा और विस्तार से वर्णन करें (कम से कम कुछ शब्द)।",
      errorLoginFirst: "कृपया पहले लॉग इन करें।",
      errorGenerateFailed: "योजना नहीं बन सकी। कृपया पुनः प्रयास करें।",
      lockedTitle: "सिनेरियो मोड",
      lockedDescription: "आपकी सटीक स्थिति के लिए एक कस्टम AI बातचीत योजना — दूतावास साक्षात्कार, विदेश में नौकरी, भोजन वितरण, कुछ भी। अपनी इच्छानुसार कई योजनाएँ बनाने के लिए मासिक सदस्यता लें।",
      lockedFeature1: "अपनी स्थिति का वर्णन करें और एक कस्टम योजना प्राप्त करें",
      lockedFeature2: "दैनिक AI वॉयस बातचीत अभ्यास",
      lockedFeature3: "अपनी इच्छानुसार कई नई परिदृश्य योजनाएँ बनाएँ (मासिक सदस्यता)",
      subscribeButton: "सदस्यता लें — $13/माह",
      myPlansTitle: "मेरी सिनेरियो योजनाएं",
      myPlansSubtitle: "आपके द्वारा बनाई गई सभी योजनाएं",
      myPlansEmpty: "आपने अभी तक कोई योजना नहीं बनाई है।",
      newPlanButton: "नई योजना",
      continueButton: "जारी रखें",
      reviewButton: "समीक्षा करें",
      generatingStatus: "योजना बन रही है...",
      errorStatus: "बनाया नहीं जा सका",
      buildingPlanTitle: "AI आपकी योजना बना रहा है...",
      buildingPlanSubtitle: "इसमें कुछ सेकंड लगते हैं।",
      buildFailedTitle: "योजना नहीं बन सकी",
      tryAgainButton: "पुनः प्रयास करें",
      slowGenerationTitle: "इसमें सामान्य से अधिक समय लग रहा है",
      slowGenerationSubtitle: "सर्वर में देरी हो सकती है। कृपया पुनः प्रयास करें।",
      dayPlanLabel: "दिन",
      todayBadge: "आज",
      startConversation: "बातचीत शुरू करें",
      completedBadge: "पूरा हुआ",
      planCompleteTitle: "आपकी {days}-दिवसीय योजना पूरी हो गई है! 🎉",
      planCompleteDescription: "अब आपके पास {language} में बुनियादी आत्मविश्वास है। अगला कदम: गहन, संरचित पाठों के लिए हमारे संरचित प्रो पथ में शामिल हों।",
      exploreProPath: "प्रो पथ देखें",
      newScenarioButton: "एक नया सिनेरियो बनाएँ",
      buildingDayTitle: "दिन {day} की सामग्री बन रही है...",
      buildingDaySubtitle: "इसमें कुछ सेकंड लगते हैं।",
      dayLoadFailedTitle: "दिन {day} की सामग्री लोड नहीं हो सकी",
      subscriptionRequiredTitle: "इस दिन को अनलॉक करने के लिए सदस्यता आवश्यक है",
      subscriptionRequiredDescription: "आपकी योजना बन गई थी, लेकिन दैनिक सामग्री बनाने के लिए आपको एक सक्रिय सिनेरियो मोड सदस्यता की आवश्यकता है।",
      phraseCounter: "वाक्यांश {current} / {total}",
      recordPrompt: "रिकॉर्ड करने के लिए टैप करें",
      recordingPrompt: "अब बोलें... पूरा होने पर स्टॉप टैप करें",
      youSaid: "आपने कहा:",
      matchConfidence: "~मैच आत्मविश्वास:",
      matchDisclaimer: "(यह उच्चारण सटीकता नहीं है — यह इस बात का अनुमान है कि व्हिस्पर ने आपके भाषण को लक्षित पाठ से मेल खाते हुए कितनी स्पष्ट रूप से समझा)",
      retryNeeded: "पुनः प्रयास करें — जारी रखने के लिए आपको {min}% से ऊपर के स्कोर की आवश्यकता है।",
      micPermissionError: "माइक्रोफ़ोन तक पहुँच नहीं हो सकी। कृपया अपने ब्राउज़र की अनुमतियाँ जाँचें।",
      transcribeError: "ट्रांसक्रिप्शन प्राप्त नहीं हो सका। कृपया पुनः प्रयास करें।",
      nextPhrase: "अगला वाक्यांश",
      dayComplete: "दिन {day} पूरा करें",
      subscriptionStatusActive: "आपकी सिनेरियो मोड सदस्यता सक्रिय है।",
      subscriptionStatusInactive: "आपके पास अभी तक सक्रिय सिनेरियो मोड सदस्यता नहीं है।",
      subscriptionExpiry: "अगला नवीनीकरण/समाप्ति:",
      cancelHowTitle: "मैं अपनी सदस्यता कैसे रद्द करूँ?",
      cancelStep1: '1. सब्सक्राइब करते समय लेमनस्क्यूज़ी से प्राप्त रसीद ईमेल देखें — इसमें "अपनी सदस्यता प्रबंधित करें" लिंक है।',
      cancelStep2: "2. या app.lemonsqueezy.com/my-orders पर जाएं और उस ईमेल से लॉग इन करें जिसका उपयोग आपने चेकआउट के समय किया था।",
      cancelStep3: "3. वहां से आप किसी भी समय रद्द कर सकते हैं — आपसे अगले बिलिंग चक्र के लिए शुल्क नहीं लिया जाएगा।",
    },
    languagesAvailable: "भाषाएँ उपलब्ध हैं",
    heroTitle: "प्रवासी श्रमिकों के लिए भाषा कौशल विकसित करना",
    heroSub: "प्रवासी श्रमिकों के लिए संरचित भाषा उत्तरजीविता पथ।",
    startBtn: "👉 अभी शुरू करें",
    viewPaths: "उपलब्ध भाषाएं देखें",
    journeyTitle: "आपकी यात्रा यहाँ से शुरू होती है",
    journeyLeaving: "घर छोड़ना",
    journeyArriving: "पहुँचना",
    journeyWorking: "काम करना",
    journeySpeaking: "बोलना",
    journeyBelonging: "अपनापन",
    redesign_title: "Just 10 minutes a day can change everything.",
    redesign_subtitle: "10 minutes a day — speak confidently in a foreign country in 84 days.",
    redesign_cta_text: "The language that makes life easier in a foreign country.",
    redesign_cta_button: "Start here",
    poster: {
        title: "आवश्यक वाक्य - व्यवस्थित शिक्षण पथ",
        subtitle: "दैनिक जीवन के लिए • काम के लिए",
        tagline: "unfold your opportunities"
    },
    scenario1Title: "आपातकाल",
    scenario1Desc: "आपका बच्चा बीमार है। आपको डॉक्टर को लक्षण समझाने की ज़रूरत है। क्या आप शब्द जानते हैं?",
    scenario1Btn: "चिकित्सा शब्दावली सीखें",
    scenario2Title: "बैंक और पैसा",
    scenario2Desc: "आपको एक बैंक खाता खोलना है। फ़ॉर्म एक विदेशी भाषा में है। क्या आप इसे भर सकते हैं?",
    scenario2Btn: "संख्या और दस्तावेज़ सीखें",
    scenario3Title: "दैनिक जीवन",
    scenario3Desc: "आपको काम पर जाने के लिए सही बस लेनी है। ड्राइवर अंग्रेज़ी नहीं बोलता। क्या आप मदद मांग सकते हैं?",
    scenario3Btn: "जीवन रक्षा वाक्यांश सीखें",
    path1Badge: "यहाँ से शुरू करें - अनुशंसित",
    path1Title: "जीवन रक्षा मार्ग",
    path1Desc: "नए देश में पहले दिन से जीवित रहने के लिए ज़रूरी वाक्यांश।",
    path2Title: "वर्णमाला मार्ग",
    path2Desc: "स्थानीय भाषा में संकेत, मेनू और संदेश पढ़ें।",
    path3Title: "संख्या मार्ग",
    path3Desc: "हर दिन पैसे, समय और संख्याओं को आत्मविश्वास से संभालें।",
    pathDetails: "व्यावहारिक उपयोग के लिए संरचित पाठ।",
    startFree: "मुफ्त शुरू करें",
    getStarted: "शुरू करें",
     features: {
      ai: { title: "AI-संचालित", desc: "आपके लिए बनाए गए वास्तविक पाठ" },
      audio: { title: "ऑडियो अंतर्निहित", desc: "हर शब्द का देशी उच्चारण सुनें" },
      dialogues: { title: "वास्तविक संवाद", desc: "वास्तविक बातचीत जो आप रोजाना करेंगे" },
      exercises: { title: "दैनिक व्यायाम", desc: "रिक्त स्थान भरें, MCQ, शब्द मिलान खेल" },
      streak: { title: "स्ट्रीक प्रणाली", desc: "दैनिक स्ट्रीक्स से प्रेरित रहें" },
      progress: { title: "प्रगति ट्रैकिंग", desc: "देखें कि आप हर हफ्ते कितनी दूर आ गए हैं" },
      certs: { title: "प्रमाणपत्र", desc: "अपनी प्रगति का प्रमाण अर्जित करें" },
      langs: { title: "21 भाषाएँ", desc: "यूरोपीय, एशियाई, मध्य पूर्वी" },
    },
    pricingTitle: "अपनी पूरी क्षमता अनलॉक करें",
    pricingSub: "आपके मुफ्त परीक्षण में जीवन रक्षा पथ का सप्ताह 1 शामिल है। सभी पाठ अनलॉक करने के लिए अपग्रेड करें।",
    pricingPage: {
        purchasingFor: "आप खरीद रहे हैं:",
        changeLanguageNote: "अपने डैशबोर्ड पर लक्ष्य भाषा बदलें।",
        autoRenewNote: "यह योजना स्वतः नवीनीकृत होती है। कभी भी रद्द करें।",
        freeTrialNote: "जीवन रक्षा पथ के सप्ताह 1 तक मुफ्त पहुँच प्राप्त करें। क्रेडिट कार्ड की आवश्यकता नहीं है।",
    },
    freePlan: {
      title: "मुफ्त योजना",
      description: "जीवन रक्षा पथ के सप्ताह 1 तक मुफ्त पहुँच प्राप्त करें। क्रेडिट कार्ड की आवश्यकता नहीं है।",
      feat1: "जीवन रक्षा पथ का सप्ताह 1",
      feat2: "प्रति दिन 5 नए शब्द",
      btn: "मुफ्त में शुरू करें"
    },
    weeklyPlan: {
      title: "साप्ताहिक योजना",
      badge: "लोकप्रिय",
      price_usd: "$3.99/सप्ताह",
      price_lkr: "LKR 1,200/सप्ताह",
      feat1: "Access to one chosen language",
      feat2: "All 3 learning paths for that language",
      feat3: "All interactive exercises & audio",
      feat4: "Cancel anytime",
      note: "12 सप्ताह के बाद भुगतान स्वचालित रूप से बंद हो जाता है।",
    },
    completePlan: {
      title: "Survival Pack",
      badge: "मूल्य",
      price_usd: "$26 one-time",
      price_lkr: "LKR 11,700",
      feat1: "Survival + Alphabet + Numbers paths",
      feat2: "Lifetime access to one language course",
      feat3: "Full 12-week curriculum",
      feat4: "Email support",
    },
    lifetimePlan: {
      title: "लाइफटाइम प्रो",
      badge: "⭐ सर्वोत्तम मूल्य",
      price_usd: "$49 एक बार",
      price_lkr: "LKR 29,700",
      feat1: "Everything in Survival Pack + Pro lessons",
      feat2: "सबको 21 भाषाओं तक पूर्ण पहुँच",
      feat3: "Priority Support",
      feat4: "Become a polyglot!",
      feat5: "Free future updates",
    },
    footerTagline: "आधुनिक प्रवासी के लिए भाषा जीवन रक्षा।",
    footerLinks: {
      company: "कंपनी",
      paths: "पथ",
      pricing: "मूल्य निर्धारण",
      privacy: "गोपनीयता नीति",
      terms: "सेवा की शर्तें",
      language: "भाषा",
    },
    footerCredit: "© 2026 LingoForge। प्रवासियों के लिए, यात्रा को समझने वाले लोगों द्वारा बनाया गया।",
    dashboard: {
      title: "डैशबोर्ड",
      welcome: "वापस स्वागत है",
      ready: "अपनी भाषा यात्रा जारी रखने के लिए तैयार हैं? चलिए इसे करते हैं।",
      currentStreak: "वर्तमान स्ट्रीक",
      keepFlame: "ज्वाला जलाए रखें!",
      xpPoints: "XP अंक",
      toNextLevel: "स्तर पर जाने के लिए",
      level: "स्तर",
      advancing: "आप आगे बढ़ रहे हैं!",
      currentPath: "वर्तमान पथ",
      language: "भाषा",
      continueJourney: "अपनी यात्रा जारी रखें",
      continueDesc: "आप {language} में {path} पथ पर हैं। जहाँ से छोड़ा था वहीं से शुरू करें।",
      nextLesson: "अगला पाठ: सप्ताह {week}, दिन {day}",
      keepProgress: "प्रगति करते रहें!",
      goToNextLesson: "अगले पाठ पर जाएँ",
      weeklyProgress: "इस सप्ताह की प्रगति",
      days: {
        mon: 'सोम',
        tue: 'मंगल',
        wed: 'बुध',
        thu: 'गुरु',
        fri: 'शुक्र',
        sat: 'शनि',
        sun: 'रवि',
      },
      reminders: {
        title: "अध्ययन अनुस्मारक",
        description: "अपनी शिक्षा के साथ ट्रैक पर बने रहने के लिए एक दैनिक सूचना सेट करें।",
        dailyReminder: "दैनिक अनुस्मारक",
        permissionNeeded: "अनुस्मारक सक्षम करने के लिए अधिसूचना अनुमति आवश्यक है।",
        grantPermission: "अनुमति दें",
        save: "अनुस्मारक सहेजें",
        saved: "अनुस्मारक सेटिंग्स सहेजी गईं!",
      },
      referrals: {
        title: "मित्र को रेफर करें",
        description: "अपना कोड किसी मित्र के साथ साझा करें। जब वे साइन अप करते हैं, तो आप दोनों को एक बोनस मिलता है!",
        yourCode: "आपका रेफरल कोड",
        copyCode: "कोड कॉपी करें",
        copied: "कॉपी किया गया!",
        bonus: "आप दोनों को 1 सप्ताह का मुफ्त प्रीमियम एक्सेस मिलता है!"
      },
      iSpeak: "मैं बोलता हूँ",
      iAmLearning: "मैं सीख रहा हूँ",
      survivalBundle: {
        title: "सर्वाइवल बंडल",
        description: "व्यावहारिक प्रवाह के लिए आपका पहला कदम।"
      },
      alphabetPath: "वर्णमाला पथ",
      numbersPath: "संख्या पथ",
      proBundle: {
        badge: "एआई उन्नत पथ",
        title: "लिंगोफोर्ज प्रो",
        description: "प्रवासी श्रमिकों के लिए एक विशेष 30-दिवसीय एकीकरण पाठ्यक्रम।",
        nextLesson: "अगला प्रो पाठ",
        exploreMap: "पाठ का नक्शा देखें",
        items: {
          rights: { title: "अधिकार और दस्तावेज़", desc: "कानूनी और नागरिक मार्गदर्शन" },
          integration: { title: "एकीकरण", desc: "सांस्कृतिक और सामाजिक मानदंड" },
          vocab: { title: "कानूनी शब्दावली", desc: "आवश्यक कानूनी शर्तें" },
          tutors: { title: "एआई ट्यूटर", desc: "एआई के साथ अभ्यास करें" }
        }
      }
    },
    notifications: {
      title: "आपके लिंगोफोर्ज पाठ का समय!",
      body: "आपका दैनिक {language} पाठ प्रतीक्षा कर रहा है। अपनी स्ट्रीक जारी रखें!",
    },
    wordCard: {
      phonetic: 'ध्वन्यात्मक',
      meaning: 'अर्थ',
      example: 'उदाहरण',
    },
    ui: {
      // survival page
      survivalPath: "जीवन रक्षा पथ",
      survivalPathDesc: "दैनिक जीवन के लिए आवश्यक वाक्यांशों में महारत हासिल करने के लिए 12-सप्ताह की यात्रा।",
      week: "सप्ताह",
      locked: "बंद",
      days: "दिन",
      completed: "पूरा हुआ",
      day: "दिन",
      upgradePlan: "योजना अपग्रेड करें",
      lessonsLocked: "इस सप्ताह के पाठ बंद हैं।",
      upgradeToUnlock: "अनलॉक करने के लिए अपग्रेड करें",
      // lesson page errors
      errorTitle: "पाठ लोड नहीं हो सका",
      errorWeekNotFound: "{path} पथ के सप्ताह {week} के लिए पाठ सामग्री मौजूद नहीं है। कृपया 'public{lessonPath}' पर फ़ाइल बनाएँ।",
      errorDayNotFound: "फ़ाइल '{lessonPath}' में दिन {day} के लिए डेटा नहीं मिला। सुनिश्चित करें कि JSON फ़ाइल में 'days' ऐरे में इस दिन के लिए एक प्रविष्टि है।",
      errorInvalidJson: "'{lessonPath}' पर पाठ फ़ाइल में अमान्य JSON है। कृपया सिंटैक्स त्रुटियों के लिए फ़ाइल की जाँच करें। त्रुटि: {error}",
      errorGeneric: "पाठ फ़ाइल लोड करने में विफल। स्थिति: {status}",
      errorInvalidDay: 'अमान्य दिन पैरामीटर: "{day}"। एक संख्या होनी चाहिए।',
      backToDashboard: "डैशबोर्ड पर वापस",
      contentNotAvailableTitle: "सामग्री उपलब्ध नहीं है",
      contentNotAvailableDesc: "यह पाठ अभी तक नहीं बनाया गया है। आप `public/lessons` डायरेक्टरी में पाठ फ़ाइल बना सकते हैं। निर्देशों के लिए कृपया `docs/lesson-generation-guide.md` देखें।",
      // lesson client page
      weekProgress: "सप्ताह की प्रगति",
      dayProgress: "दिन की प्रगति",
      vocabulary: "शब्दावली",
      noVocabulary: "इस पाठ के लिए कोई शब्दावली नहीं है।",
      dialogues: "संवाद",
      tipsAndCulture: "टिप्स और संस्कृति",
      pronunciationTip: "उच्चारण टिप",
      culturalNote: "सांस्कृतिक नोट",
      dayComplete: "दिन पूरा!",
      earnedXP: "आपने {xp} XP और एक {streak_bonus} स्ट्रीक बोनस अर्जित किया है।",
      completeDay: "दिन पूरा करें (+{xp} XP)",
      complete50Percent: "समाप्त करने के लिए कम से कम 50% अभ्यास पूरा करें।",
      // exercises
      practiceExercises: "अभ्यास",
      fillInTheBlanks: "रिक्त स्थान भरें",
      correct: "सही!",
      matchingPairs: "जोड़ियां मिलाओ",
      greatJob: "बहुत बढ़िया!",
      allPairsMatched: "सभी जोड़ियां सही ढंग से मिलाई गईं!",
      arrangeTheWords: "शब्दों को व्यवस्थित करें",
      translate: 'अनुवाद करें: "{hint}"',
      check: "जाँचें",
      reset: "रीसेट",
      correctSentence: 'सही! वाक्य है: "{sentence}"',
      tryAgain: "बिलकुल नहीं। फिर से कोशिश करें!",
      // navigation
      pathsAndFeatures: "पथ और सुविधाएँ",
      logIn: "लॉग इन करें",
      signUp: "साइन अप करें",
      profile: "प्रोफ़ाइल",
      logOut: "लॉग आउट",
    }
  },
  Urdu: {
    scenario: {
      pageTitle: "سیناریو موڈ",
      pageDescription: 'اپنی حقیقی زندگی کی صورتحال بیان کریں — AI روزانہ کی گفتگو کا منصوبہ بناتا ہے جو اس کے مطابق ہو۔ (مثلاً "میں اٹلی کے ایک ریستوراں میں ویٹر کی نوکری شروع کر رہا ہوں" ، "رومانیہ میں ورک پرمٹ سفارت خانے کے انٹرویو کی تیاری کر رہا ہوں")',
      myPlans: "میرے منصوبے",
      situationLabel: "آپ کی صورتحال",
      situationPlaceholder: "اپنی صورتحال یہاں بیان کریں...",
      targetLanguageLabel: "ہدف کی زبان",
      nativeLanguageLabel: "مادری زبان",
      generateButton: "میرا منصوبہ بنائیں",
      generatingButton: "منصوبہ بن رہا ہے...",
      errorTooShort: "براہ کرم اپنی صورتحال کو تھوڑا مزید تفصیل سے بیان کریں (کم از کم چند الفاظ)۔",
      errorLoginFirst: "براہ کرم پہلے لاگ ان کریں۔",
      errorGenerateFailed: "منصوبہ تیار نہیں ہو سکا۔ براہ کرم دوبارہ کوشش کریں۔",
      lockedTitle: "سیناریو موڈ",
      lockedDescription: "آپ کی صحیح صورتحال کے لیے ایک کسٹم AI گفتگو کا منصوبہ — سفارت خانے کا انٹرویو ، بیرون ملک ملازمت ، کھانے کی ترسیل ، کچھ بھی۔ ماہانہ سبسکرپشن لے کر جتنے چاہیں منصوبے بنائیں۔",
      lockedFeature1: "اپنی صورتحال بیان کریں اور ایک کسٹم منصوبہ حاصل کریں",
      lockedFeature2: "روزانہ AI آواز کے ساتھ گفتگو کی مشق",
      lockedFeature3: "جتنے چاہیں نئے منظر نامے کے منصوبے بنائیں (ماہانہ سبسکرپشن)",
      subscribeButton: "سبسکرائب کریں — $13/ماہ",
      myPlansTitle: "میرے سیناریو منصوبے",
      myPlansSubtitle: "آپ کے بنائے ہوئے تمام منصوبے",
      myPlansEmpty: "آپ نے ابھی تک کوئی منصوبہ نہیں بنایا ہے۔",
      newPlanButton: "نیا منصوبہ",
      continueButton: "جاری رکھیں",
      reviewButton: "جائزہ لیں",
      generatingStatus: "منصوبہ بن رہا ہے...",
      errorStatus: "تیار نہیں کیا جا سکا",
      buildingPlanTitle: "AI آپ کا منصوبہ بنا رہا ہے...",
      buildingPlanSubtitle: "اس میں چند سیکنڈ لگتے ہیں۔",
      buildFailedTitle: "منصوبہ تیار نہیں ہو سکا",
      tryAgainButton: "دوبارہ کوشش کریں",
      slowGenerationTitle: "اس میں معمول سے زیادہ وقت لگ رہا ہے",
      slowGenerationSubtitle: "سرور میں تاخیر ہو سکتی ہے۔ براہ کرم دوبارہ کوشش کریں۔",
      dayPlanLabel: "دن",
      todayBadge: "آج",
      startConversation: "گفتگو شروع کریں",
      completedBadge: "مکمل",
      planCompleteTitle: "آپ کا {days} روزہ منصوبہ مکمل ہو گیا ہے! 🎉",
      planCompleteDescription: "اب آپ کے پاس {language} میں بنیادی اعتماد ہے۔ اگلا مرحلہ: گہرے ، منظم اسباق کے لیے ہمارے پرو پاتھ میں شامل ہوں۔",
      exploreProPath: "پرو پاتھ دریافت کریں",
      newScenarioButton: "ایک نیا سیناریو بنائیں",
      buildingDayTitle: "دن {day} کا مواد بن رہا ہے...",
      buildingDaySubtitle: "اس میں چند سیکنڈ لگتے ہیں۔",
      dayLoadFailedTitle: "دن {day} کا مواد لوڈ نہیں ہو سکا",
      subscriptionRequiredTitle: "اس دن کو غیر مقفل کرنے کے لیے سبسکرپشن درکار ہے",
      subscriptionRequiredDescription: "آپ کا منصوبہ بن گیا تھا ، لیکن روزانہ کا مواد تیار کرنے کے لیے آپ کو ایک فعال سیناریو موڈ سبسکرپشن کی ضرورت ہے۔",
      phraseCounter: "جملہ {current} / {total}",
      recordPrompt: "ریکارڈ کرنے کے لیے ٹیپ کریں",
      recordingPrompt: "اب بولیں... مکمل ہونے پر سٹاپ دبائیں",
      youSaid: "آپ نے کہا:",
      matchConfidence: "~میچ کا اعتماد:",
      matchDisclaimer: "(یہ تلفظ کی درستگی نہیں ہے — یہ اس بات کا تخمینہ ہے کہ Whisper نے آپ کی بات کو ہدف کے متن سے میل کھاتے ہوئے کتنا واضح طور پر سمجھا)",
      retryNeeded: "دوبارہ کوشش کریں — جاری رکھنے کے لیے آپ کو {min}% سے زیادہ اسکور کی ضرورت ہے۔",
      micPermissionError: "مائیکروفون تک رسائی نہیں ہو سکی۔ براہ کرم اپنے براؤزر کی اجازتیں چیک کریں۔",
      transcribeError: "ٹرانسکرپشن حاصل نہیں ہو سکی۔ براہ کرم دوبارہ کوشش کریں۔",
      nextPhrase: "اگلا جملہ",
      dayComplete: "دن {day} مکمل کریں",
      subscriptionStatusActive: "آپ کی سیناریو موڈ سبسکرپشن فعال ہے۔",
      subscriptionStatusInactive: "آپ کے پاس ابھی تک فعال سیناریو موڈ سبسکرپشن نہیں ہے۔",
      subscriptionExpiry: "اگلی تجدید/میعاد ختم ہونے کی تاریخ:",
      cancelHowTitle: "میں اپنی سبسکرپشن کیسے منسوخ کروں؟",
      cancelStep1: '1. سبسکرائب کرتے وقت لیمن اسکوئیزی سے موصول ہونے والی رسید ای میل چیک کریں — اس میں "Manage your subscription" کا لنک ہے۔',
      cancelStep2: "2. یا app.lemonsqueezy.com/my-orders پر جائیں اور اس ای میل سے لاگ ان کریں جو آپ نے چیک آؤٹ پر استعمال کی تھی۔",
      cancelStep3: "3. وہاں سے آپ کسی بھی وقت منسوخ کر سکتے ہیں — آپ سے اگلے بلنگ سائیکل کے لیے چارج نہیں لیا جائے گا۔",
    },
    languagesAvailable: "زبانیں دستیاب ہیں۔",
    heroTitle: "Developing Language Skills for Migrant Workers",
    heroSub: "مہاجر کارکنوں کے لیے منظم زبان کی بقا کے راستے۔",
    startBtn: "👉 ابھی شروع کریں",
    viewPaths: "دستیاب زبانیں دیکھیں",
    journeyTitle: "آپ کا سفر یہاں سے شروع ہوتا ہے۔",
    journeyLeaving: "گھر چھوڑنا",
    journeyArriving: "پہنچنا",
    journeyWorking: "کام کرنا",
    journeySpeaking: "بولنا",
    journeyBelonging: "اپنانا",
    redesign_title: "Just 10 minutes a day can change everything.",
    redesign_subtitle: "10 minutes a day — speak confidently in a foreign country in 84 days.",
    redesign_cta_text: "The language that makes life easier in a foreign country.",
    redesign_cta_button: "Start here",
    poster: {
        title: "ضروری جملے - منظم سیکھنے کا راستہ",
        subtitle: "روزمرہ کی زندگی کے لیے • کام کے لیے",
        tagline: "unfold your opportunities"
    },
    scenario1Title: "ہنگامی صورتحال",
    scenario1Desc: "آپ کا بچہ بیمار ہے۔ آپ کو ڈاکٹر کو علامات بتانے کی ضرورت ہے۔ کیا آپ کو الفاظ آتے ہیں؟",
    scenario1Btn: "طبی الفاظ سیکھیں",
    scenario2Title: "بینک اور پیسہ",
    scenario2Desc: "آپ کو بینک اکاؤنٹ کھولنے کی ضرورت ہے۔ فارم غیر ملکی زبان میں ہے۔ کیا آپ اسے بھر سکتے ہیں؟",
    scenario2Btn: "نمبر اور دستاویزات سیکھیں",
    scenario3Title: "روزمرہ زندگی",
    scenario3Desc: "آپ کو کام پر جانے کے لیے صحیح بس لینی ہے۔ ڈرائیور انگریزی نہیں بولتا۔ کیا آپ مدد مانگ سکتے ہیں؟",
    scenario3Btn: "بقا کے جملے سیکھیں",
    path1Badge: "یہاں سے شروع کریں - تجویز کردہ",
    path1Title: "بقا کا راستہ",
    path1Desc: "نئے ملک میں پہلے دن سے زندہ رہنے کے لیے ضروری جملے۔",
    path2Title: "حروف تہجی کا راستہ",
    path2Desc: "مقامی زبان میں نشانیاں، مینو اور پیغامات پڑھیں۔",
    path3Title: "اعداد کا راستہ",
    path3Desc: "ہر روز پیسے، وقت اور اعداد کو اعتماد سے سنبھالیں۔",
    pathDetails: "عملی استعمال کے لیے منظم اسباق۔",
    startFree: "مفت شروع کریں",
    getStarted: "شروع کریں",
    features: {
      ai: { title: "AI سے چلنے والا", desc: "آپ کے لیے تیار کردہ حقیقی اسباق" },
      audio: { title: "آڈیو شامل", desc: "ہر لفظ کا مقامی تلفظ سنیں" },
      dialogues: { title: "حقیقی مکالمے", desc: "اصل گفتگو جو آپ روزانہ استعمال کریں گے" },
      exercises: { title: "روزانہ مشقیں", desc: "خالی جگہیں پر کریں، MCQ، الفاظ ملانے والے کھیل" },
      streak: { title: "اسٹریک سسٹم", desc: "روزانہ اسٹریک کے ساتھ متحرک رہیں" },
      progress: { title: "پیشرفت سے باخبر رہنا", desc: "دیکھیں کہ آپ ہر ہفتے کتنی ترقی کر چکے ہیں" },
      certs: { title: "سرٹیفکیٹس", desc: "آپنی پیشرفت کا ثبوت حاصل کریں" },
      langs: { title: "21 زبانیں", desc: "یورپی، ایشیائی، مشرق وسطیٰ" },
    },
    pricingTitle: "اپنی پوری صلاحیت کو غیر مقفل کریں۔",
    pricingSub: "آپ کے مفت ٹرائل میں بقا کے راستے کا ہفتہ 1 شامل ہے۔ تمام اسباق کو غیر مقفل کرنے کے لیے اپ گریڈ کریں۔",
    pricingPage: {
        purchasingFor: "آپ خرید رہے ہیں:",
        changeLanguageNote: "آپنے ڈیش بورڈ پر ہدف کی زبان تبدیل کریں۔",
        autoRenewNote: "یہ منصوبہ خود بخود تجدید ہو جاتا ہے۔ کسی بھی وقت منسوخ کریں۔",
        freeTrialNote: "بقا کے راستے کے ہفتہ 1 تک مفت رسائی حاصل کریں۔ کریڈٹ کارڈ کی ضرورت نہیں ہے۔",
    },
    freePlan: {
      title: "مفت منصوبہ",
      description: "بقا کے راستے کے ہفتہ 1 تک مفت رسائی حاصل کریں۔ کریڈٹ کارڈ کی ضرورت نہیں ہے۔",
      feat1: "بقا کے راستے کا پہلا ہفتہ",
      feat2: "روزانہ 5 نئے الفاظ",
      btn: "مفت میں شروع کریں"
    },
    weeklyPlan: {
      title: "ہفتہ وار منصوبہ",
      badge: "مقبول",
      price_usd: "$3.99/ہفتہ",
      price_lkr: "LKR 1,200/ہفتہ",
      feat1: "Access to one chosen language",
      feat2: "All 3 learning paths for that language",
      feat3: "All interactive exercises & audio",
      feat4: "Cancel anytime",
      note: "12 ہفتوں کے بعد ادائیگیاں خود بخود رک جاتی ہیں۔",
    },
    completePlan: {
      title: "Survival Pack",
      badge: "قیمت",
      price_usd: "$26 one-time",
      price_lkr: "LKR 11,700",
      feat1: "Survival + Alphabet + Numbers paths",
      feat2: "Lifetime access to one language course",
      feat3: "Full 12-week curriculum",
      feat4: "Email support",
    },
    lifetimePlan: {
      title: "لائف ٹائم پرو",
      badge: "⭐ بہترین قیمت",
      price_usd: "$49 یک مشت",
      price_lkr: "LKR 29,700",
      feat1: "Everything in Survival Pack + Pro lessons",
      feat2: "تمام 21 زبانوں تک مکمل رسائی",
      feat3: "Priority Support",
      feat4: "Become a polyglot!",
      feat5: "مفت مستقبل کے اپ ڈیٹس",
    },
    footerTagline: "جدید مہاجر کے لیے زبان کی بقا۔",
    footerLinks: {
      company: "کمپنی",
      paths: "راستے",
      pricing: "قیمتیں",
      privacy: "رازداری کی پالیسی",
      terms: "سروس کی شرائط",
      language: "زبان",
    },
    footerCredit: "© 2026 LingoForge۔ مہاجرین کے لیے، ان لوگوں کے ذریعے بنایا گیا جو سفر کو سمجھتے ہیں۔",
    dashboard: {
      title: "ڈیش بورڈ",
      welcome: "خوش آمدید",
      ready: "کیا آپ اپنا لسانی سفر جاری رکھنے کے لیے تیار ہیں؟ چلو کرتے ہیں۔",
      currentStreak: "موجودہ اسٹریک",
      keepFlame: "روزانہ کم از کم 5 الفاظ سیکھنے کی کوشش کریں۔",
      xpPoints: "XP پوائنٹس",
      toNextLevel: "سطح پر جانے کے لئے",
      level: "سطح",
      advancing: "آپ ترقی کر رہے ہیں!",
      currentPath: "موجودہ راستہ",
      language: "زبان",
      continueJourney: "اپنا سفر جاری رکھیں",
      continueDesc: "آپ {language} میں {path} کے راستے پر ہیں۔ وہیں سے شروع کریں جہاں سے آپ نے چھوڑا تھا۔",
      nextLesson: "اگلا سبق: ہفتہ {week}، دن {day}",
      keepProgress: "ترقی کرتے رہیں!",
      goToNextLesson: "اگلے سبق پر جائیں",
      weeklyProgress: "اس ہفتے کی پیشرفت",
      days: {
        mon: 'پیر',
        tue: 'منگل',
        wed: 'بدھ',
        thu: 'جمعرات',
        fri: 'جمعہ',
        sat: 'ہفتہ',
        sun: 'اتوار',
      },
      reminders: {
        title: "مطالعی یاد دہانیاں",
        description: "اپنی تعلیم کے ساتھ ٹریک پر رہنے کے لئے روزانہ اطلاع مرتب کریں۔",
        dailyReminder: "روزانہ یاد دہانی",
        permissionNeeded: "یاد دہانیوں کو فعال کرنے کے لئے اطلاع کی اجازت درکار ہے۔",
        grantPermission: "اجازت دیں",
        save: "یاد دہانی محفوظ کریں",
        saved: "یاد دہانی کی ترتیبات محفوظ ہوگئیں!",
      },
       referrals: {
        title: "دوست کو ریفر کریں",
        description: "اپنا کوڈ دوست کے ساتھ شیئر کریں۔ جب وہ سائن اپ کریں گے تو آپ دونوں کو بونس ملے گا!",
        yourCode: "آپ کا ریفرل کوڈ",
        copyCode: "کوڈ کاپی کریں",
        copied: "کاپی ہو گیا!",
        bonus: "آپ دونوں کو 1 ہفتے کی مفت پریمیم رسائی ملے گی!"
      },
      iSpeak: "میں بولتا ہوں",
      iAmLearning: "میں سیکھ رہا ہوں",
      survivalBundle: {
        title: "سروائیول بنڈل",
        description: "عملی روانی کی طرف آپ کا پہلا قدم۔"
      },
      alphabetPath: "حروف تہجی کا راستہ",
      numbersPath: "اعداد کا راستہ",
      proBundle: {
        badge: "AI ایڈوانسڈ پاتھ",
        title: "LingoForge پرو",
        description: "مہاجر کارکنوں کے لیے ایک خصوصی 30 روزہ انضمام نصاب۔",
        nextLesson: "اگلا پرو سبق",
        exploreMap: "سبق کا نقشہ دریافت کریں",
        items: {
          rights: { title: "حقوق اور دستاویزات", desc: "قانونی اور شہری رہنمائی" },
          integration: { title: "انضمام", desc: "ثقافتی اور سماجی اصول" },
          vocab: { title: "قانونی الفاظ", desc: "ضروری قانونی اصطلاحات" },
          tutors: { title: "AI ٹیوٹرز", desc: "AI کے ساتھ مشق کریں" }
        }
      }
    },
    notifications: {
        title: "آپ کے لنگوفورج سبق کا وقت!",
        body: "آپ کا روزانہ {language} سبق انتظار کر رہا ہے۔ اپنی اسٹریک جاری رکھیں!",
    },
    wordCard: {
      phonetic: 'صوتیات',
      meaning: 'معنی',
      example: 'مثال',
    },
    ui: {
      // survival page
      survivalPath: "بقا کا راستہ",
      survivalPathDesc: "روزمرہ کی زندگی کے لئے ضروری جملے سیکھنے کے لئے 12 ہفتوں کا سفر۔",
      week: "ہفتہ",
      locked: "مقفل",
      days: "دن",
      completed: "مکمل",
      day: "دن",
      upgradePlan: "منصوبہ اپ گریڈ کریں",
      lessonsLocked: "اس ہفتے کے اسباق مقفل ہیں۔",
      upgradeToUnlock: "انلاک کرنے کے لیے اپ گریڈ کریں۔",
      // lesson page errors
      errorTitle: "سبق لوڈ نہیں ہو سکا",
      errorWeekNotFound: "{path} راستے کے ہفتہ {week} کے لئے سبق کا مواد موجود نہیں ہے۔ براہ کرم 'public{lessonPath}' پر فائل بنائیں۔",
      errorDayNotFound: "فائل '{lessonPath}' میں دن {day} کا ڈیٹا نہیں ملا۔ یقینی بنائیں کہ JSON فائل میں 'days' سرنی میں اس دن کے لئے ایک اندراج موجود ہے۔",
      errorInvalidJson: "'{lessonPath}' پر سبق کی فائل میں غلط JSON ہے۔ براہ کرم نحو کی غلطیوں کے لئے فائل کو چیک کریں۔ خرابی: {error}",
      errorGeneric: "سبق کی فائل لوڈ کرنے میں ناکام۔ حیثیت: {status}",
      errorInvalidDay: 'غلط دن کا پیرامیٹر: "{day}"۔ ایک نمبر ہونا چاہئے۔',
      backToDashboard: "ڈیش بورڈ پر واپس",
      contentNotAvailableTitle: "مواد دستیاب نہیں ہے",
      contentNotAvailableDesc: "یہ سبق ابھی تک نہیں بنایا گیا ہے۔ آپ `public/lessons` ڈائرکٹری میں سبق की فائل بنا سکتے ہیں۔ ہدایات کے لئے براہ کرم `docs/lesson-generation-guide.md` دیکھیں۔",
      // lesson client page
      weekProgress: "ہفتے کی پیشرفت",
      dayProgress: "دن کی پیشرفت",
      vocabulary: "الفاظ",
      noVocabulary: "اس سبق کے لئے کوئی الفاظ نہیں ہیں۔",
      dialogues: "مکالمے",
      tipsAndCulture: "تجاویز اور ثقافت",
      pronunciationTip: "تلفظ کی ترکیب",
      culturalNote: "ثقافتی نوٹ",
      dayComplete: "دن مکمل!",
      earnedXP: "آپ نے {xp} XP اور {streak_bonus} کا اسٹریک بونس حاصل کیا ہے۔",
      completeDay: "دن مکمل کریں (+{xp} XP)",
      complete50Percent: "ختم کرنے کے لئے کم از-کم 50٪ مشقیں مکمل کریں۔",
      // exercises
      practiceExercises: "مشقیں",
      fillInTheBlanks: "خالی جگہیں پر کریں",
      correct: "درست!",
      matchingPairs: "جوڑے ملائیں",
      greatJob: "بہت اچھا!",
      allPairsMatched: "تمام جوڑے صحیح طریقے سے ملائے گئے!",
      arrangeTheWords: "الفاظ ترتیب دیں",
      translate: 'ترجمہ کریں: "{hint}"',
      check: "چیک کریں",
      reset: "ری سیٹ کریں",
      correctSentence: 'درست! جملہ ہے: "{sentence}"',
      tryAgain: "ٹھیک نہیں ہے۔ دوبارہ کوشش کریں!",
      // navigation
      pathsAndFeatures: "راستے اور خصوصیات",
      logIn: "لاگ ان کریں",
      signUp: "سائن اپ کریں",
      profile: "پروفائل",
      logOut: "لاگ آؤٹ",
    }
  },
  Bengali: {
    scenario: {
      pageTitle: "সিনারিओ মোড",
      pageDescription: 'আপনার বাস্তব জীবনের পরিস্থিতির বর্ণনা দিন — AI একটি দৈনিক কথোপকথন পরিকল্পনা তৈরি করে যা এর সাথে খাপ খায়। (যেমন "আমি ইতালির একটি রেস্তোরাঁয় ওয়েটারের কাজ শুরু করছি", "রোমানিয়ায় ওয়ার্ক পারমিট দূতাবাস ইন্টারভিউয়ের জন্য প্রস্তুতি নিচ্ছি")',
      myPlans: "আমার পরিকল্পনা",
      situationLabel: "আপনার পরিস্থিতি",
      situationPlaceholder: "আপনার পরিস্থিতি এখানে বর্ণনা করুন...",
      targetLanguageLabel: "টার্গেট ভাষা",
      nativeLanguageLabel: "মাতৃভাষা",
      generateButton: "আমার প্ল্যান তৈরি করুন",
      generatingButton: "আপনার প্ল্যান তৈরি হচ্ছে...",
      errorTooShort: "অনুগ্রহ করে আপনার পরিস্থিতি আরও বিস্তারিতভাবে বর্ণনা করুন (অন্তত কয়েকটা শব্দ)।",
      errorLoginFirst: "অনুগ্রহ করে আগে লগ ইন করুন।",
      errorGenerateFailed: "প্ল্যান তৈরি করা যায়নি। অনুগ্রহ করে আবার চেষ্টা করুন।",
      lockedTitle: "সিনারিও মোড",
      lockedDescription: "আপনার সঠিক পরিস্থিতির জন্য একটি কাস্টম AI কথোপকথন পরিকল্পনা — দূতাবাস ইন্টারভিউ, বিদেশে চাকরি, খাবার ডেলিভারি, যেকোনো কিছু। আপনার ইচ্ছামতো অনেক পরিকল্পনা তৈরি করতে মাসিক সাবস্ক্রিপশন নিন।",
      lockedFeature1: "আপনার পরিস্থিতি বর্ণনা করুন এবং একটি কাস্টম প্ল্যান পান",
      lockedFeature2: "দৈনিক AI ভয়েস কথোপকথন অনুশীলন",
      lockedFeature3: "আপনার ইচ্ছামতো অনেক নতুন সিনারিও প্ল্যান তৈরি করুন (মাসিক সাবস্ক্রিপশন)",
      subscribeButton: "সাবস্ক্রাইব করুন — $13/মাস",
      myPlansTitle: "আমার সিনারিও প্ল্যান",
      myPlansSubtitle: "আপনার তৈরি করা সমস্ত পরিকল্পনা",
      myPlansEmpty: "আপনি এখনও কোনো পরিকল্পনা তৈরি করেননি।",
      newPlanButton: "নতুন প্ল্যান",
      continueButton: "চালিয়ে যান",
      reviewButton: "পর্যালোচনা",
      generatingStatus: "প্ল্যান তৈরি হচ্ছে...",
      errorStatus: "তৈরি করা যায়নি",
      buildingPlanTitle: "AI আপনার প্ল্যান তৈরি করছে...",
      buildingPlanSubtitle: "এতে কয়েক সেকেন্ড সময় লাগে।",
      buildFailedTitle: "প্ল্যান তৈরি করা যায়নি",
      tryAgainButton: "আবার চেষ্টা করুন",
      slowGenerationTitle: "এতে স্বাভাবিকের চেয়ে বেশি সময় লাগছে",
      slowGenerationSubtitle: "সার্ভারে বিলম্ব হতে পারে। অনুগ্রহ করে আবার চেষ্টা করুন।",
      dayPlanLabel: "দিন",
      todayBadge: "আজ",
      startConversation: "কথোপকথন শুরু করুন",
      completedBadge: "সম্পন্ন",
      planCompleteTitle: "আপনার {days}-দিনের প্ল্যান সম্পূর্ণ! 🎉",
      planCompleteDescription: "এখন আপনার {language} ভাষায় প্রাথমিক আত্মবিশ্বাস আছে। পরবর্তী ধাপ: গভীর, সুসংগঠিত পাঠের জন্য আমাদের কাঠামোগত প্রো পথে যোগ দিন।",
      exploreProPath: "প্রো পথ দেখুন",
      newScenarioButton: "একটি নতুন সিনারিও তৈরি করুন",
      buildingDayTitle: "দিন {day}-এর কন্টেন্ট তৈরি হচ্ছে...",
      buildingDaySubtitle: "এতে কয়েক সেকেন্ড সময় লাগে।",
      dayLoadFailedTitle: "দিন {day}-এর কন্টেন্ট লোড করা যায়নি",
      subscriptionRequiredTitle: "এই দিনটি আনলক করতে সাবস্ক্রিপশন প্রয়োজন",
      subscriptionRequiredDescription: "আপনার প্ল্যান তৈরি করা হয়েছিল, কিন্তু দৈনিক কন্টেন্ট তৈরি করতে আপনার একটি সক্রিয় সিনারিও মোড সাবস্ক্রিপশন প্রয়োজন।",
      phraseCounter: "বাক্যাংশ {current} / {total}",
      recordPrompt: "রেকর্ড করতে ট্যাপ করুন",
      recordingPrompt: "এখন কথা বলুন... শেষ হলে স্টপ ট্যাপ করুন",
      youSaid: "আপনি বলেছেন:",
      matchConfidence: "~ম্যাচ আত্মবিশ্বাস:",
      matchDisclaimer: "(এটি উচ্চারণ সঠিকতা নয় — এটি একটি অনুমান যে হুইস্পার আপনার কথাকে টার্গেট টেক্সটের সাথে কতটা স্পষ্টভাবে মিলিয়েছে)",
      retryNeeded: "আবার চেষ্টা করুন — চালিয়ে যেতে আপনার {min}%-এর উপরে স্কোর প্রয়োজন।",
      micPermissionError: "মাইক্রোফোন অ্যাক্সেস করা যায়নি। অনুগ্রহ করে আপনার ব্রাউজার পারমিশন চেক করুন।",
      transcribeError: "ট্রান্সক্রিপশন পাওয়া যায়নি। অনুগ্রহ করে আবার চেষ্টা করুন।",
      nextPhrase: "পরবর্তী বাক্যাংশ",
      dayComplete: "দিন {day} সম্পূর্ণ করুন",
      subscriptionStatusActive: "আপনার সিনারিও মোড সাবস্ক্রিপশন সক্রিয় আছে।",
      subscriptionStatusInactive: "আপনার এখনও সক্রিয় সিনারিও মোড সাবস্ক্রিপশন নেই।",
      subscriptionExpiry: "পরবর্তী রিনিউয়াল/মেয়াদ শেষ:",
      cancelHowTitle: "আমি আমার সাবস্ক্রিপশন কীভাবে বাতিল করব?",
      cancelStep1: '১. সাবস্ক্রাইব করার সময় লেমনস্কুইজি থেকে পাঠানো রসিদ ইমেলটি দেখুন — এতে "Manage your subscription" লিঙ্ক আছে।',
      cancelStep2: "২. অথবা app.lemonsqueezy.com/my-orders এ যান এবং চেকআউটের সময় ব্যবহৃত ইমেল দিয়ে লগ ইন করুন।",
      cancelStep3: "৩. সেখান থেকে আপনি যেকোনো সময় বাতিল করতে পারেন — পরবর্তী বিলিং সাইকেলের জন্য আপনার কাছে কোনো চার্জ নেওয়া হবে না।",
    },
    languagesAvailable: "ভাষা উপলব্ধ আছে",
    heroTitle: "Developing Language Skills for Migrant Workers",
    heroSub: "অভিবাসী কর্মীদের জন্য পরিকল্পিত ভাষা টিকে থাকার পথ।",
    startBtn: "👉 এখন শুরু করুন",
    viewPaths: "উপলব্ধ ভাষাগুলি দেখুন",
    journeyTitle: "আপনার যাত্রা এখান থেকে শুরু",
    journeyLeaving: "বাড়ি ছাড়া",
    journeyArriving: "আগমন",
    journeyWorking: "কাজ করা",
    journeySpeaking: "কথা বলা",
    journeyBelonging: "একাত্ম হওয়া",
    redesign_title: "Just 10 minutes a day can change everything.",
    redesign_subtitle: "10 minutes a day — speak confidently in a foreign country in 84 days.",
    redesign_cta_text: "The language that makes life easier in a foreign country.",
    redesign_cta_button: "Start here",
    poster: {
        title: "প্রয়োজনীয় বাক্য - পদ্ধতিগত শেখার পথ",
        subtitle: "দৈনন্দিন জীবনের জন্য • কাজের জন্য",
        tagline: "unfold your opportunities"
    },
    scenario1Title: "জরুরী অবস্থা",
    scenario1Desc: "আপনার সন্তান অসুস্থ। আপনাকে একজন ডাক্তারকে উপসর্গ ব্যাখ্যা করতে হবে। আপনি কি শব্দগুলো জানেন?",
    scenario1Btn: "চিকিৎসা শব্দভান্ডার শিখুন",
    scenario2Title: "ব্যাঙ্ক ও টাকা",
    scenario2Desc: "আপনাকে একটি ব্যাঙ্ক অ্যাকাউন্ট খুলতে হবে। ফর্মটি একটি বিদেশী ভাষায় আছে। আপনি কি এটি পূরণ করতে পারবেন?",
    scenario2Btn: "সংখ্যা ও নথি শিখুন",
    scenario3Title: "দৈনন্দিন জীবন",
    scenario3Desc: "আপনাকে কাজে যাওয়ার জন্য সঠিক বাস ধরতে হবে। ড্রাইভার ইংরেজি বলে না। আপনি কি সাহায্য চাইতে পারবেন?",
    scenario3Btn: "বেঁচে থাকার বাক্যাংশ শিখুন",
    path1Badge: "এখান থেকে শুরু করুন - প্রস্তাবিত",
    path1Title: "বেঁচে থাকার পথ",
    path1Desc: "নতুন দেশে প্রথম দিন থেকে বেঁচে থাকতে প্রয়োজনীয় বাক্যাংশ।",
    path2Title: "বর্ণমালার পথ",
    path2Desc: "স্থানীয় ভাষায় চিহ্ন, মেনু এবং বার্তা পড়ুন।",
    path3Title: "সংখ্যার পথ",
    path3Desc: "প্রতিদিন টাকা, সময় এবং সংখ্যা আত্মবিশ্বাসের সাথে সামলান।",
    pathDetails: "ব্যবহারিক ব্যবহারের জন্য পরিকল্পিত পাঠ।",
    startFree: "বিনামূল্যে শুরু করুন",
    getStarted: "শুরু করুন",
     features: {
      ai: { title: "AI-চালিত", desc: "আপনার জন্য তৈরি বাস্তব পাঠ" },
      audio: { title: "অডিও বিল্ট-ইন", desc: "প্রতিটি শব্দের জন্য স্থানীয় উচ্চারণ শুনুন" },
      dialogues: { title: "বাস্তব সংলাপ", desc: "প্রকৃত কথোপকথন যা আপনি প্রতিদিন ব্যবহার করবেন" },
      exercises: { title: "দৈনিক অনুশীলন", desc: "ফাঁকা স্থান পূরণ, MCQ, শব্দ মেলানো খেলা" },
      streak: { title: "স্ট্রিক সিস্টেম", desc: "দৈনিক স্ট্রিকের সাথে অনুপ্রাণিত থাকুন" },
      progress: { title: "অগ্রগতি ট্র্যাকিং", desc: "দেখুন প্রতি সপ্তাহে আপনি কতটা এগিয়েছেন" },
      certs: { title: "সার্টিফিকেট", desc: "আপনার অগ্রগতির প্রমাণ অর্জন করুন" },
      langs: { title: "21টি ভাষা", desc: "ইউরোপীয়, এশিয়ান, মধ্যপ্রাচ্য" },
    },
    pricingTitle: "আপনার সম্পূর্ণ সম্ভাবনা আনলক করুন",
    pricingSub: "আপনার বিনামূল্যে ট্রায়ালে বেঁচে থাকার পথের সপ্তাহ 1 অন্তর্ভুক্ত। সমস্ত পাঠ আনলক করতে আপগ্রেড করুন।",
    pricingPage: {
        purchasingFor: "আপনি কিনছেন:",
        changeLanguageNote: "আপনার ড্যাশবোর্ডে লক্ষ্য ভাষা পরিবর্তন করুন।",
        autoRenewNote: "এই পরিকল্পনা স্বয়ংক্রিয়ভাবে পুনর্নবীকরণ হয়। যেকোনো সময় বাতিল করুন।",
        freeTrialNote: "বেঁচে থাকার পথের সপ্তাহ 1 বিনামূল্যে অ্যাক্সেস করুন। কোন ক্রেডিট কার্ডের প্রয়োজন নেই।",
    },
    freePlan: {
      title: "বিনামূল্যে পরিকল্পনা",
      description: "বেঁচে থাকার পথের সপ্তাহ 1 বিনামূল্যে অ্যাক্সেস করুন। কোন ক্রেডিট কার্ডের প্রয়োজন নেই।",
      feat1: "বেঁচে থাকার পথের সপ্তাহ 1",
      feat2: "প্রতিদিন 5টি নতুন শব্দ",
      btn: "বিনামূল্যে শুরু করুন"
    },
    weeklyPlan: {
      title: "সাপ্তাহিক পরিকল্পনা",
      badge: "জনপ্রিয়",
      price_usd: "$3.99/সপ্তাহ",
      price_lkr: "LKR 1,200/সপ্তাহ",
      feat1: "Access to one chosen language",
      feat2: "All 3 learning paths for that language",
      feat3: "All interactive exercises & audio",
      feat4: "Cancel anytime",
      note: "12 সপ্তাহ পরে পেমেন্ট স্বয়ংক্রিয়ভাবে বন্ধ হয়ে যায়।",
    },
    completePlan: {
      title: "Survival Pack",
      badge: "মূল্য",
      price_usd: "$26 one-time",
      price_lkr: "LKR 11,700",
      feat1: "Survival + Alphabet + Numbers paths",
      feat2: "Lifetime access to one language course",
      feat3: "Full 12-week curriculum",
      feat4: "Email support",
    },
    lifetimePlan: {
      title: "আজীবন প্রো",
      badge: "⭐ সেরা মূল্য",
      price_usd: "$49 এককালীন",
      price_lkr: "LKR 29,700",
      feat1: "Everything in Survival Pack + Pro lessons",
      feat2: "সব ২০টি ভাষায় সম্পূর্ণ অ্যাক্সেস",
      feat3: "Priority Support",
      feat4: "Become a polyglot!",
      feat5: "বিনামূল্যে ভবিষ্যতের আপডেট",
    },
    footerTagline: "আধুনিক অভিবাসীর জন্য ভাষা টিকে থাকা।",
    footerLinks: {
      company: "প্রতিষ্ঠান",
      paths: "পথ",
      pricing: "মূল্য নির্ধারণ",
      privacy: "গোপনীয়তা নীতি",
      terms: "পরিষেবার শর্তাবলী",
      language: "ভাষা",
    },
    footerCredit: "© 2026 LingoForge। অভিবাসীদের জন্য, যারা যাত্রা বোঝেন তাদের দ্বারা নির্মিত।",
    dashboard: {
      title: "ড্যাশবোর্ড",
      welcome: "আবার স্বাগতম",
      ready: "আপনার ভাষা যাত্রা চালিয়ে যেতে প্রস্তুত? চলুন শুরু করা যাক।",
      currentStreak: "বর্তমান স্ট্রিক",
      keepFlame: "শিখা জ্বালিয়ে রাখুন!",
      xpPoints: "XP পয়েন্ট",
      toNextLevel: "স্তরে যেতে",
      level: "স্তর",
      advancing: "আপনি এগিয়ে যাচ্ছেন!",
      currentPath: "বর্তমান পথ",
      language: "ভাষা",
      continueJourney: "আপনার যাত্রা চালিয়ে যান",
      continueDesc: "আপনি {language}-এ {path} পথে আছেন। যেখান থেকে ছেড়েছিলেন সেখান থেকে শুরু করুন।",
      nextLesson: "পরবর্তী পাঠ: সপ্তাহ {week}, দিন {day}",
      keepProgress: "অগ্রগতি করতে থাকুন!",
      goToNextLesson: "পরবর্তী পাঠে যান",
      weeklyProgress: "এই সপ্তাহের অগ্রগতি",
      days: {
        mon: 'সোম',
        tue: 'মঙ্গল',
        wed: 'বুধ',
        thu: 'বৃহ',
        fri: 'শুক্র',
        sat: 'শনি',
        sun: 'রবি',
      },
      reminders: {
        title: "অধ্যয়ন অনুস্মারক",
        description: "আপনার শেখার সাথে ট্র্যাকে থাকতে একটি দৈনিক বিজ্ঞপ্তি সেট করুন।",
        dailyReminder: "দৈনিক অনুস্মারক",
        permissionNeeded: "অনুস্মারক সক্ষম করতে বিজ্ঞপ্তি অনুমতি প্রয়োজন।",
        grantPermission: "অনুমতি দিন",
        save: "অনুস্মারক সংরক্ষণ করুন",
        saved: "অনুস্মারক সেটিংস সংরক্ষিত হয়েছে!",
      },
       referrals: {
        title: "বন্ধুকে রেফার করুন",
        description: "আপনার কোড বন্ধুর সাথে শেয়ার করুন। তারা সাইন আপ করলে আপনারা দুজনই বোনাস পাবেন!",
        yourCode: "আপনার রেফারেল কোড",
        copyCode: "কোড কপি করুন",
        copied: "কপি করা হয়েছে!",
        bonus: "আপনারা দুজনেই ১ সপ্তাহের বিনামূল্যে প্রিমিয়াম অ্যাক্সেস পাবেন!"
      },
      iSpeak: "আমি কথা বলি",
      iAmLearning: "আমি শিখছি",
      survivalBundle: {
        title: "সারভাইভাল বান্ডেল",
        description: "ব্যবহারিক সাবলীলতার জন্য আপনার প্রথম পদক্ষেপ।"
      },
      alphabetPath: "বর্ণমালা পথ",
      numbersPath: "সংখ্যা পথ",
      proBundle: {
        badge: "এআই অ্যাডভান্সড পাথ",
        title: "লিঙ্গোফোর্জ প্রো",
        description: "অভিবাসী কর্মীদের জন্য একটি বিশেষ 30 দিনের ইন্টিগ্রেশন পাঠ্যক্রম।",
        nextLesson: "পরবর্তী প্রো পাঠ",
        exploreMap: "পাঠের মানচিত্র অন্বেষণ করুন",
        items: {
          rights: { title: "অধিকার ও নথি", desc: "আইনি এবং নাগরিক নির্দেশিকা" },
          integration: { title: "একীকরণ", desc: "সাংস্কৃতিক এবং সামাজিক নিয়ম" },
          vocab: { title: "আইনি শব্দভান্ডার", desc: "অপরিহার্য আইনি পরিভাষা" },
          tutors: { title: "এআই টিউটর", desc: "এআই দিয়ে অনুশীলন করুন" }
        }
      }
    },
    notifications: {
      title: "আপনার লিঙ্গোফোর্জ পাঠের জন্য সময়!",
      body: "আপনার দৈনিক {language} পাঠ অপেক্ষা করছে। আপনার স্ট্রিক চালিয়ে যান!",
    },
    wordCard: {
      phonetic: 'উচ্চারণ',
      meaning: 'অর্থ',
      example: 'উদাহরণ',
    },
    ui: {
      // survival page
      survivalPath: "বেঁচে থাকার পথ",
      survivalPathDesc: "দৈনন্দিন জীবনের জন্য প্রয়োজনীয় বাক্যাংশগুলি আয়ত্ত করার জন্য একটি 12-সপ্তাহের যাত্রা।",
      week: "সপ্তাহ",
      locked: "লক করা",
      days: "দিন",
      completed: "সম্পন্ন",
      day: "দিন",
      upgradePlan: "পরিকল্পনা আপগ্রেড করুন",
      lessonsLocked: "এই সপ্তাহের পাঠগুলি লক করা আছে।",
      upgradeToUnlock: "আনলক করতে আপগ্রেড করুন",
      // lesson page errors
      errorTitle: "পাঠ লোড করা যায়নি",
      errorWeekNotFound: "{path} পথের সপ্তাহ {week} এর জন্য পাঠের বিষয়বস্তু বিদ্যমান নেই। অনুগ্রহ করে 'public{lessonPath}' এ ফাইলটি তৈরি করুন।",
      errorDayNotFound: "ফাইল '{lessonPath}' এ দিন {day} এর জন্য ডেটা খুঁজে পাওয়া যায়নি। নিশ্চিত করুন যে JSON ফাইলের 'days' অ্যারেতে এই দিনের জন্য একটি এন্ট্রি রয়েছে।",
      errorInvalidJson: "'{lessonPath}' এ পাঠ ফাইলটিতে অবৈধ JSON রয়েছে। অনুগ্রহ করে সিনট্যাক্স ত্রুটির জন্য ফাইলটি পরীক্ষা করুন। ত্রুটি: {error}",
      errorGeneric: "পাঠ ফাইল লোড করতে ব্যর্থ হয়েছে। স্থিতি: {status}",
      errorInvalidDay: 'অবৈধ দিন প্যারামিটার: "{day}"। এটি একটি সংখ্যা হতে হবে।',
      backToDashboard: "ড্যাশবোর্ডে ফিরে যান",
      contentNotAvailableTitle: "বিষয়বস্তু উপলব্ধ নেই",
      contentNotAvailableDesc: "এই পাঠটি এখনও তৈরি করা হয়নি। আপনি `public/lessons` ডিরেক্টরিতে পাঠ ফাইল তৈরি করতে পারেন। নির্দেশাবলীর জন্য অনুগ্রহ করে `docs/lesson-generation-guide.md` দেখুন।",
      // lesson client page
      weekProgress: "সপ্তাহের অগ্রগতি",
      dayProgress: "দিনের অগ্রগতি",
      vocabulary: "শব্দভান্ডার",
      noVocabulary: "এই পাঠের জন্য কোন শব্দভান্ডার নেই।",
      dialogues: "সংলাপ",
      tipsAndCulture: "টিপস এবং সংস্কৃতি",
      pronunciationTip: "উচ্চারণ টিপ",
      culturalNote: "সাংস্কৃতিক নোট",
      dayComplete: "দিন সম্পন্ন!",
      earnedXP: "আপনি {xp} XP এবং একটি {streak_bonus} স্ট্রিক বোনাস অর্জন করেছেন।",
      completeDay: "দিন সম্পন্ন করুন (+{xp} XP)",
      complete50Percent: "শেষ করতে কমপক্ষে 50% অনুশীলন সম্পন্ন করুন।",
      // exercises
      practiceExercises: "অনুশীলন",
      fillInTheBlanks: "ফাঁকা স্থান পূরণ করুন",
      correct: "সঠিক!",
      matchingPairs: "জোড়া মিলান",
      greatJob: "দারুণ!",
      allPairsMatched: "সব জোড়া সঠিকভাবে মিলেছে!",
      arrangeTheWords: "শব্দগুলি সাজান",
      translate: 'অনুবাদ করুন: "{hint}"',
      check: "চেক করুন",
      reset: "রিসেট করুন",
      correctSentence: 'সঠিক! বাক্যটি হল: "{sentence}"',
      tryAgain: "ঠিক হয়নি। আবার চেষ্টা করুন!",
      // navigation
      pathsAndFeatures: "পথ এবং বৈশিষ্ট্য",
      logIn: "লগ ইন করুন",
      signUp: "সাইন আপ করুন",
      profile: "প্রোফাইল",
      logOut: "লগ আউট",
    }
  },
  Nepali: {
    scenario: {
      pageTitle: "सिनारियो मोड",
      pageDescription: 'आफ्नो वास्तविक जीवनको अवस्था वर्णन गर्नुहोस् — AI ले एक दैनिक कुराकानी योजना बनाउँछ जुन यसमा फिट हुन्छ। (जस्तै "म इटालीको एक रेस्टुरेन्टमा वेटरको काम सुरु गर्दैछु", "रोमानियामा वर्क परमिट दूतावास अन्तर्वार्ताको लागि तयारी गर्दैछु")',
      myPlans: "मेरा योजनाहरू",
      situationLabel: "तपाईंको अवस्था",
      situationPlaceholder: "यहाँ आफ्नो अवस्था वर्णन गर्नुहोस्...",
      targetLanguageLabel: "लक्ष्य भाषा",
      nativeLanguageLabel: "मातृभाषा",
      generateButton: "मेरो योजना बनाउनुहोस्",
      generatingButton: "तपाईंको योजना बन्दैछ...",
      errorTooShort: "कृपया आफ्नो अवस्थालाई अलि विस्तारमा वर्णन गर्नुहोस् (कम्तिमा केही शब्दहरू)।",
      errorLoginFirst: "कृपया पहिले लग इन गर्नुहोस्।",
      errorGenerateFailed: "योजना बनाउन सकिएन। कृपया पुनः प्रयास गर्नुहोस्।",
      lockedTitle: "सिनारियो मोड",
      lockedDescription: "तपाईंको सटीक अवस्थाको लागि एक कस्टम AI कुराकानी योजना — दूतावास अन्तर्वार्ता, विदेशमा जागिर, खाना डेलिभरी, जे पनि। आफ्नो इच्छा अनुसार धेरै योजनाहरू बनाउन मासिक सदस्यता लिनुहोस्।",
      lockedFeature1: "आफ्नो अवस्था वर्णन गर्नुहोस् र एक कस्टम योजना प्राप्त गर्नुहोस्",
      lockedFeature2: "दैनिक AI आवाज कुराकानी अभ्यास",
      lockedFeature3: "आफ्नो इच्छा अनुसार धेरै नयाँ परिदृश्य योजनाहरू बनाउनुहोस् (मासिक सदस्यता)",
      subscribeButton: "सदस्यता लिनुहोस् — $13/महिना",
      myPlansTitle: "मेरा सिनारियो योजनाहरू",
      myPlansSubtitle: "तपाईंले बनाउनुभएका सबै योजनाहरू",
      myPlansEmpty: "तपाईंले अहिलेसम्म कुनै योजना बनाउनुभएको छैन।",
      newPlanButton: "नयाँ योजना",
      continueButton: "जारी राख्नुहोस्",
      reviewButton: "समीक्षा गर्नुहोस्",
      generatingStatus: "योजना बन्दैछ...",
      errorStatus: "बनाउन सकिएन",
      buildingPlanTitle: "AI ले तपाईंको योजना बनाउँदैछ...",
      buildingPlanSubtitle: "यसले केही सेकेन्ड लिन्छ।",
      buildFailedTitle: "योजना बनाउन सकिएन",
      tryAgainButton: "पुनः प्रयास गर्नुहोस्",
      slowGenerationTitle: "यसले सामान्य भन्दा बढी समय लिइरहेको छ",
      slowGenerationSubtitle: "सर्भरमा ढिलाइ हुन सक्छ। कृपया पुनः प्रयास गर्नुहोस्।",
      dayPlanLabel: "दिन",
      todayBadge: "आज",
      startConversation: "कुराकानी सुरु गर्नुहोस्",
      completedBadge: "पूरा भयो",
      planCompleteTitle: "तपाईंको {days}-दिने योजना पूरा भयो! 🎉",
      planCompleteDescription: "अब तपाईंसँग {language} मा आधारभूत आत्मविश्वास छ। अर्को चरण: गहिरो, संरचित पाठहरूको लागि हाम्रो संरचित प्रो मार्गमा सामेल हुनुहोस्।",
      exploreProPath: "प्रो मार्ग हेर्नुहोस्",
      newScenarioButton: "नयाँ सिनारियो बनाउनुहोस्",
      buildingDayTitle: "दिन {day} को सामग्री बन्दैछ...",
      buildingDaySubtitle: "यसले केही सेकेन्ड लिन्छ।",
      dayLoadFailedTitle: "दिन {day} को सामग्री लोड हुन सकेन",
      subscriptionRequiredTitle: "यो दिन अनलक गर्न सदस्यता आवश्यक छ",
      subscriptionRequiredDescription: "तपाईंको योजना बनाइएको थियो, तर दैनिक सामग्री उत्पन्न गर्न तपाईंलाई सक्रिय सिनारियो मोड सदस्यता चाहिन्छ।",
      phraseCounter: "वाक्यांश {current} / {total}",
      recordPrompt: "रेकर्ड गर्न ट्याप गर्नुहोस्",
      recordingPrompt: "अहिले बोल्नुहोस्... पूरा भएपछि स्टप ट्याप गर्नुहोस्",
      youSaid: "तपाईंले भन्नुभयो:",
      matchConfidence: "~मैच आत्मविश्वास:",
      matchDisclaimer: "(यो उच्चारण शुद्धता होइन — यो ह्विस्परले तपाईंको बोलीलाई लक्षित पाठसँग कत्तिको स्पष्ट रूपमा मिलायो भन्ने अनुमान हो)",
      retryNeeded: "पुनः प्रयास गर्नुहोस् — जारी राख्न तपाईंलाई {min}% भन्दा माथिको स्कोर चाहिन्छ।",
      micPermissionError: "माइक्रोफोनमा पहुँच हुन सकेन। कृपया आफ्नो ब्राउजर अनुमतिहरू जाँच गर्नुहोस्।",
      transcribeError: "ट्रान्सक्रिप्शन प्राप्त हुन सकेन। कृपया पुनः प्रयास गर्नुहोस्।",
      nextPhrase: "अर्को वाक्यांश",
      dayComplete: "दिन {day} पूरा गर्नुहोस्",
      subscriptionStatusActive: "तपाईंको सिनारियो मोड सदस्यता सक्रिय छ।",
      subscriptionStatusInactive: "तपाईंसँग अहिलेसम्म सक्रिय सिनारियो मोड सदस्यता छैन।",
      subscriptionExpiry: "अर्को नवीकरण/समाप्ति:",
      cancelHowTitle: "म मेरो सदस्यता कसरी रद्द गरुँ?",
      cancelStep1: '१. सब्सक्राइब गर्दा लेमनस्क्यूजीबाट प्राप्त रसिद इमेल हेर्नुहोस् — यसमा "Manage your subscription" लिङ्क छ।',
      cancelStep2: "२. वा app.lemonsqueezy.com/my-orders मा जानुहोस् र चेकआउटको समयमा प्रयोग गर्नुभएको इमेलबाट लग इन गर्नुहोस्।",
      cancelStep3: "३. त्यहाँबाट तपाईं कुनै पनि समय रद्द गर्न सक्नुहुन्छ — तपाईंलाई अर्को बिलिङ चक्रको लागि शुल्क लिइने छैन।",
    },
    languagesAvailable: "भाषाहरू उपलब्ध छन्",
    heroTitle: "Developing Language Skills for Migrant Workers",
    heroSub: "प्रवासी कामदारहरूका लागि संरचित भाषा जीवन रक्षा मार्गहरू।",
    startBtn: "👉 अब सुरु गर्नुहोस्",
    viewPaths: "उपलब्ध भाषाहरू हेर्नुहोस्",
    journeyTitle: "तपाईंको यात्रा यहाँबाट सुरु हुन्छ",
    journeyLeaving: "घर छोड्दै",
    journeyArriving: "आगमन",
    journeyWorking: "काम गर्दै",
    journeySpeaking: "बोल्दै",
    journeyBelonging: "सम्बन्धित",
    redesign_title: "Just 10 minutes a day can change everything.",
    redesign_subtitle: "10 minutes a day — speak confidently in a foreign country in 84 days.",
    redesign_cta_text: "The language that makes life easier in a foreign country.",
    redesign_cta_button: "Start here",
    poster: {
        title: "आवश्यक वाक्यहरू - व्यवस्थित सिकाइ मार्ग",
        subtitle: "दैनिक जीवनको लागि • कामको लागि",
        tagline: "unfold your opportunities"
    },
    scenario1Title: "आपतकालिन",
    scenario1Desc: "तपाईंको बच्चा बिरामी छ। तपाईंले डाक्टरलाई लक्षणहरू व्याख्या गर्न आवश्यक छ। के तपाईंलाई शब्दहरू थाहा छ?",
    scenario1Btn: "चिकित्सा शब्दावली सिक्नुहोस्",
    scenario2Title: "बैंक र पैसा",
    scenario2Desc: "तपाईंले बैंक खाता खोल्न आवश्यक छ। फारम विदेशी भाषामा छ। के तपाईं यसलाई भर्न सक्नुहुन्छ?",
    scenario2Btn: "संख्या र कागजातहरू सिक्नुहोस्",
    scenario3Title: "दैनिक जीवन",
    scenario3Desc: "तपाईंले काममा जानको लागि सही बस लिन आवश्यक छ। ड्राइभरले अंग्रेजी बोल्दैन। के तपाईं मद्दत माग्न सक्नुहुन्छ?",
    scenario3Btn: "जीवन रक्षा वाक्यांशहरू सिक्नुहोस्",
    path1Badge: "यहाँबाट सुरु गर्नुहोस् - सिफारिस गरिएको",
    path1Title: "जीवन रक्षा मार्ग",
    path1Desc: "नयाँ देशमा पहिलो दिनदेखि बाँच्नको लागि आवश्यक वाक्यांशहरू।",
    path2Title: "वर्णमाला मार्ग",
    path2Desc: "स्थानीय भाषा में संकेत, मेनु र सन्देशहरू पढ्नुहोस्।",
    path3Title: "संख्या मार्ग",
    path3Desc: "हरेक दिन पैसा, समय र संख्याहरू आत्मविश्वासका साथ ह्यान्डल गर्नुहोस्।",
    pathDetails: "व्यावहारिक प्रयोगको लागि संरचित पाठहरू।",
    startFree: "नि:शुल्क सुरु गर्नुहोस्",
    getStarted: "सुरु गर्नुहोस्",
    features: {
      ai: { title: "एआई-संचालित", desc: "तपाईंको लागि उत्पन्न वास्तविक पाठहरू" },
      audio: { title: "अडियो निर्मित", desc: "हरेक शब्दको लागि स्थानीय उच्चारण सुन्नुहोस्" },
      dialogues: { title: "वास्तविक संवादहरू", desc: "वास्तविक कुराकानीहरू जुन तपाईंले दैनिक प्रयोग गर्नुहुनेछ" },
      exercises: { title: "दैनिक अभ्यासहरू", desc: "खाली ठाउँ भर्नुहोस्, MCQ, शब्द मिलान खेलहरू" },
      streak: { title: "स्ट्रीक प्रणाली", desc: "दैनिक स्ट्रीकहरूको साथ प्रेरित रहनुहोस्" },
      progress: { title: "प्रगति ट्र्याकिङ", desc: "हेर्नुहोस् तपाईं हरेक हप्ता कति टाढा पुग्नुभएको छ" },
      certs: { title: "प्रमाणपत्रहरू", desc: "आफ्नो प्रगतिको प्रमाण कमाउनुहोस्" },
      langs: { title: "21 भाषाहरू", desc: "युरोपेली, एसियाली, मध्य पूर्वी" },
    },
    pricingTitle: "आफ्नो पूर्ण क्षमता अनलक गर्नुहोस्",
    pricingSub: "तपाईंको नि:शुल्क परीक्षणमा जीवन रक्षा मार्गको हप्ता १ समावेश छ। सबै पाठहरू अनलक गर्न अपग्रेड गर्नुहोस्।",
    pricingPage: {
        purchasingFor: "तपाईं खरिद गर्दै हुनुहुन्छ:",
        changeLanguageNote: "आफ्नो ड्यासबोर्डमा लक्ष्य भाषा परिवर्तन गर्नुहोस्।",
        autoRenewNote: "यो योजना स्वतः नवीकरण हुन्छ। जुनसुकै बेला रद्द गर्नुहोस्।",
        freeTrialNote: "जीवन रक्षा मार्गको हप्ता १ मा निःशुल्क पहुँच पाउनुहोस्। कुनै क्रेडिट कार्ड आवश्यक छैन।",
    },
    freePlan: {
      title: "नि:शुल्क योजना",
      description: "जीवन रक्षा मार्गको हप्ता १ मा निःशुल्क पहुँच पाउनुहोस्। कुनै क्रेडिट कार्ड आवश्यक छैन।",
      feat1: "जीवन रक्षा मार्गको हप्ता १",
      feat2: "प्रति दिन ५ नयाँ शब्दहरू",
      btn: "नि:शुल्क सुरु गर्नुहोस्"
    },
    weeklyPlan: {
      title: "साप्ताहिक योजना",
      badge: "लोकप्रिय",
      price_usd: "$3.99/हप्ता",
      price_lkr: "LKR 1,200/हप्ता",
      feat1: "Access to one chosen language",
      feat2: "All 3 learning paths for that language",
      feat3: "All interactive exercises & audio",
      feat4: "Cancel anytime",
      note: "12 हप्ता पछि भुक्तानीहरू स्वचालित रूपमा बन्द हुन्छन्।",
    },
    completePlan: {
      title: "Survival Pack",
      badge: "मूल्य",
      price_usd: "$26 one-time",
      price_lkr: "LKR 11,700",
      feat1: "Survival + Alphabet + Numbers paths",
      feat2: "Lifetime access to one language course",
      feat3: "Full 12-week curriculum",
      feat4: "Email support",
    },
    lifetimePlan: {
      title: "लाइफटाइम प्रो",
      badge: "⭐ उत्तम मूल्य",
      price_usd: "$49 एक पटक",
      price_lkr: "LKR 29,700",
      feat1: "Everything in Survival Pack + Pro lessons",
      feat2: "सबै २१ भाषाहरूमा पूर्ण पहुँच",
      feat3: "Priority Support",
      feat4: "Become a polyglot!",
      feat5: "नि:शुल्क भविष्यका अद्यावधिकहरू",
    },
    footerTagline: "आधुनिक प्रवासीका लागि भाषा जीवन रक्षा।",
    footerLinks: {
      company: "कम्पनी",
      paths: "मार्गहरू",
      pricing: "मूल्य निर्धारण",
      privacy: "गोपनीयता नीति",
      terms: "सेवा सर्तहरू",
      language: "भाषा",
    },
    footerCredit: "© 2026 LingoForge। प्रवासीहरूका लागि, यात्रा बुझ्ने मानिसहरूद्वारा निर्मित।",
    dashboard: {
      title: "ड्यासबोर्ड",
      welcome: "फेरि स्वागत छ",
      ready: "आफ्नो भाषा यात्रा जारी राख्न तयार हुनुहुन्छ? सुरु गरौं।",
      currentStreak: "वर्तमान स्ट्रीक",
      keepFlame: "ज्वाला जलाइराख्नुहोस्!",
      xpPoints: "XP अंक",
      toNextLevel: "स्तरमा जान",
      level: "स्तर",
      advancing: "तपाईं अगाडि बढ्दै हुनुहुन्छ!",
      currentPath: "वर्तमान मार्ग",
      language: "भाषा",
      continueJourney: "आफ्नो यात्रा जारी राख्नुहोस्",
      continueDesc: "तपाईं {language} मा {path} मार्गमा हुनुहुन्छ। जहाँ छोड्नुभएको थियो त्यहीँबाट सुरु गर्नुहोस्।",
      nextLesson: "अर्को पाठ: हप्ता {week}, दिन {day}",
      keepProgress: "प्रगति गरिरहनुहोस्!",
      goToNextLesson: "अर्को पाठमा जानुहोस्",
      weeklyProgress: "यो हप्ताको प्रगति",
      days: {
        mon: 'सोम',
        tue: 'मंगल',
        wed: 'बुध',
        thu: 'बिहि',
        fri: 'शुक्र',
        sat: 'शनि',
        sun: 'आइत',
      },
      reminders: {
        title: "अध्ययन रिमाइन्डरहरू",
        description: "आफ्नो सिकाइसँग ट्र्याकमा रहन दैनिक सूचना सेट गर्नुहोस्।",
        dailyReminder: "दैनिक रिमाइन्डर",
        permissionNeeded: "रिमाइन्डरहरू सक्षम गर्न सूचना अनुमति आवश्यक छ।",
        grantPermission: "अनुमति दिनुहोस्",
        save: "रिमाइन्डर बचत गर्नुहोस्",
        saved: "रिमाइन्डर सेटिङहरू बचत गरियो!",
      },
      referrals: {
        title: "साथीलाई रेफर गर्नुहोस्",
        description: "आफ्नो कोड साथीसँग साझा गर्नुहोस्। जब उनीहरूले साइन अप गर्छन्, तपाईं दुबैले बोनस पाउनुहुन्छ!",
        yourCode: "तपाईंको रेफरल कोड",
        copyCode: "कोड प्रतिलिपि गर्नुहोस्",
        copied: "प्रतिलिपि गरियो!",
        bonus: "तपाईं दुबैले १ हप्ताको निःशुल्क प्रिमियम पहुँच पाउनुहुन्छ!"
      },
      iSpeak: "म बोल्छु",
      iAmLearning: "म सिक्दै छु",
      survivalBundle: {
        title: "सर्वाइभल बन्डल",
        description: "व्यावहारिक प्रवाहको लागि तपाईंको पहिलो कदम।"
      },
      alphabetPath: "वर्णमाला पथ",
      numbersPath: "संख्या पथ",
      proBundle: {
        badge: "एआई उन्नत मार्ग",
        title: "लिंगोफोर्ज प्रो",
        description: "प्रवासी कामदारहरूका लागि एक विशेष ३०-दिने एकीकरण पाठ्यक्रम।",
        nextLesson: "अर्को प्रो पाठ",
        exploreMap: "पाठ नक्सा अन्वेषण गर्नुहोस्",
        items: {
          rights: { title: "अधिकार र कागजातहरू", desc: "कानूनी र नागरिक मार्गदर्शन" },
          integration: { title: "एकीकरण", desc: "सांस्कृतिक र सामाजिक मान्यताहरू" },
          vocab: { title: "कानूनी शब्दावली", desc: "आवश्यक कानूनी सर्तहरू" },
          tutors: { title: "एआई ट्युटरहरू", desc: "एआईसँग अभ्यास गर्नुहोस्" }
        }
      }
    },
    notifications: {
      title: "तपाईंको लिंगोफोर्ज पाठको लागि समय!",
      body: "तपाईंको दैनिक {language} पाठ पर्खिरहेको छ। आफ्नो स्ट्रीक जारी राख्नुहोस्!",
    },
    wordCard: {
      phonetic: 'ध्वन्यात्मक',
      meaning: 'अर्थ',
      example: 'उदाहरण',
    },
     ui: {
      // survival page
      survivalPath: "जीवन रक्षा मार्ग",
      survivalPathDesc: "दैनिक जीवनका लागि आवश्यक वाक्यांशहरूमा महारत हासिल गर्न १२-हप्ते यात्रा।",
      week: "हप्ता",
      locked: "लक गरिएको",
      days: "दिनहरू",
      completed: "पूरा भयो",
      day: "दिन",
      upgradePlan: "योजना अपग्रेड गर्नुहोस्",
      lessonsLocked: "यस हप्ताका पाठहरू लक गरिएका छन्।",
      upgradeToUnlock: "अनलक गर्न अपग्रेड गर्नुहोस्",
      // lesson page errors
      errorTitle: "पाठ लोड गर्न सकिएन",
      errorWeekNotFound: "{path} मार्गको हप्ता {week} को लागि पाठ सामग्री अवस्थित छैन। कृपया 'public{lessonPath}' मा फाइल सिर्जना गर्नुहोस्।",
      errorDayNotFound: "फाइल '{lessonPath}' मा दिन {day} को लागि डाटा फेला पार्न सकिएन। निश्चित गर्नुहोस् कि JSON फाइलको 'days' एरेमा यस दिनको लागि एउटा प्रविष्टि छ।",
      errorInvalidJson: "'{lessonPath}' मा रहेको पाठ फाइलमा अमान्य JSON छ। कृपया सिन्ट्याक्स त्रुटिहरूको लागि फाइल जाँच गर्नुहोस्। त्रुटि: {error}",
      errorGeneric: "पाठ फाइल लोड गर्न असफल भयो। स्थिति: {status}",
      errorInvalidDay: 'अमान्य दिन प्यारामिटर: "{day}"। यो एउटा संख्या हुनुपर्छ।',
      backToDashboard: "ड्यासबोर्डमा फर्कनुहोस्",
      contentNotAvailableTitle: "सामग्री उपलब्ध छैन",
      contentNotAvailableDesc: "यो पाठ अझै सिर्जना गरिएको छैन। तपाईंले `public/lessons` डाइरेक्टरीमा पाठ फाइल सिर्जना गर्न सक्नुहुन्छ। त्यसैले कृपया `docs/lesson-generation-guide.md` हेर्नुहोस्।",
      // lesson client page
      weekProgress: "हप्ताको प्रगति",
      dayProgress: "दिनको प्रगति",
      vocabulary: "शब्दावली",
      noVocabulary: "यस पाठको लागि कुनै शब्दावली छैन।",
      dialogues: "संवादहरू",
      tipsAndCulture: "सुझाव र संस्कृति",
      pronunciationTip: "उच्चारण सुझाव",
      culturalNote: "सांस्कृतिक नोट",
      dayComplete: "दिन पूरा भयो!",
      earnedXP: "तपाईंले {xp} XP र {streak_bonus} को स्ट्रीक बोनस कमाउनुभएको छ।",
      completeDay: "दिन पूरा गर्नुहोस् (+{xp} XP)",
      complete50Percent: "समाप्त गर्न कम्तिमा ५०% अभ्यास पूरा गर्नुहोस्।",
      // exercises
      practiceExercises: "अभ्यासहरू",
      fillInTheBlanks: "खाली ठाउँहरू भर्नुहोस्",
      correct: "सही!",
      matchingPairs: "जोडी मिलाउनुहोस्",
      greatJob: "राम्रो काम!",
      allPairsMatched: "सबै जोडीहरू सही रूपमा मिलाइएका छन्!",
      arrangeTheWords: "शब्दहरू मिलाउनुहोस्",
      translate: 'अनुवाद गर्नुहोस्: "{hint}"',
      check: "जाँच गर्नुहोस्",
      reset: "रिसेट गर्नुहोस्",
      correctSentence: 'सही! वाक्य हो: "{sentence}"',
      tryAgain: "ठीक भएन। फेरि प्रयास गर्नुहोस्!",
      // navigation
      pathsAndFeatures: "मार्ग र सुविधाहरू",
      logIn: "लग इन गर्नुहोस्",
      signUp: "साइन अप गर्नुहोस्",
      profile: "प्रोफाइल",
      logOut: "लग आउट गर्नुहोस्",
    }
  },
};

export const targetLanguages = [
  { lang: "German", flag: "🇩🇪", countries: ["de-DE", "Austria", "Switzerland"], alphabetSize: 30 },
  { lang: "French", flag: "🇫🇷", countries: ["fr-FR", "Belgium", "Switzerland", "Canada"], alphabetSize: 26 },
  { lang: "Italian", flag: "🇮🇹", countries: ["it-IT", "Switzerland"], alphabetSize: 21 },
  { lang: "Spanish", flag: "🇪🇸", countries: ["es-ES", "Latin America"], alphabetSize: 27 },
  { lang: "Portuguese", flag: "🇵🇹", countries: ["pt-PT", "Brazil"], alphabetSize: 26 },
  { lang: "Dutch", flag: "🇳🇱", countries: ["nl-NL", "Belgium"], alphabetSize: 26 },
  { lang: "Greek", flag: "🇬🇷", countries: ["el-GR", "Cyprus"], alphabetSize: 24 },
  { lang: "Polish", flag: "🇵🇱", countries: ["pl-PL"], alphabetSize: 32 },
  { lang: "Romanian", flag: "🇷🇴", countries: ["ro-RO"], alphabetSize: 31 },
  { lang: "Serbian", flag: "🇷🇸", countries: ["sr-RS"], alphabetSize: 30 },
  { lang: "Russian", flag: "🇷🇺", countries: ["ru-RU"], alphabetSize: 33 },
  { lang: "Finnish", flag: "🇫🇮", countries: ["fi-FI"], alphabetSize: 29 },
  { lang: "Korean", flag: "🇰🇷", countries: ["ko-KR"], alphabetSize: 24 },
  { lang: "Japanese", flag: "🇯🇵", countries: ["ja-JP"], alphabetSize: 46 }, // Hiragana
  { lang: "Arabic", flag: "🇸🇦", countries: ["ar-SA", "UAE", "Qatar", "Kuwait"], alphabetSize: 28 },
  { lang: "Hebrew", flag: "🇮🇱", countries: ["he-IL"], alphabetSize: 22 },
  { lang: "English", flag: "🇬🇧", countries: ["en-US", "Australia", "Canada", "USA", "en-GB"], alphabetSize: 26 },
  { lang: "Turkish", flag: "🇹🇷", countries: ["tr-TR"], alphabetSize: 29 },
  { lang: "Hindi", flag: "🇮🇳", countries: ["hi-IN"], alphabetSize: 46 },
  { lang: "Tamil", flag: "🇮🇳", countries: ["ta-IN", "Sri Lanka", "Singapore"], alphabetSize: 0 },
  { lang: "Chinese", flag: "🇨🇳", countries: ["zh-CN", "Singapore", "Taiwan"], alphabetSize: 0 }, // Not alphabet-based
];

export const nativeLanguages = ["English", "Sinhala", "Hindi", "Urdu", "Bengali", "Nepali"];
